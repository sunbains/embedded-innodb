...
static char *srv_add_path_separator_if_needed(
...
static os_thread_ret_t
io_handler_thread(void *arg) /*!< in: pointer to the number of the segment in
                             the aio array */
{
  ulint segment;
  ulint i;
...
@return	DB_SUCCESS or error code */
static ulint open_or_create_data_files(
  ...
  *create_new_db = FALSE;
  ...
  for (i = 0; i < srv_n_data_files; i++) {
    ...
    if (!is_absolute) {
      ...
      ut_a(ut_strlen(home) + ut_strlen(srv_data_file_names[i]) <
           (sizeof name) - 1);
