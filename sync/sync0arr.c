...
static ibool sync_array_detect_deadlock(
                      ...
                      sync_cell_t *cell)     /*!< in: sync cell */
{
  ...
  if (type == SYNC_MUTEX) {
    ...
    ib_logger(ib_stream,
              ...
              (ulong)rw_lock_get_reader_count(rwlock), (ulong)rwlock->waiters,
              rwlock->lock_word, rwlock->last_s_file_name,
              (ulong)rwlock->last_s_line, rwlock->last_x_file_name,
...
ibool sync_array_print_long_waits(void) {
  ...
  for (i = 0; i < sync_primary_wait_array->n_cells; i++) {
    ...
    if (cell->wait_object != NULL && cell->waiting &&
        difftime(time(NULL), cell->reservation_time) > fatal_timeout) {
      fatal = TRUE;
    }
  }
  ...
  if (noticed) {
    ...
    ib_logger(ib_stream, "InnoDB: ###### Diagnostic info printed"
