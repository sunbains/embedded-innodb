...
/** Truncates a table
@return	error code or DB_SUCCESS */
db_err ddl_truncate_table(
  dict_table_t *table, /*!< in: table handle */
  trx_t *trx
); /*!< in: transaction handle */
/** Renames a table.
@return	error code or DB_SUCCESS */
db_err ddl_rename_table(
...
/** Drops a database.
@return	error code or DB_SUCCESS */
db_err ddl_drop_database(
  const char *name, /*!< in: database name which
                                                ends in '/' */
  trx_t *trx
); /*!< in: transaction handle */

/** Drop all partially created indexes. */
void ddl_drop_all_temp_indexes(ib_recovery_t recovery); /*!< in: recovery level setting */

/** Drop all temporary tables. */
void ddl_drop_all_temp_tables(ib_recovery_t recovery); /*!< in: recovery level setting */
