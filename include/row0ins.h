...
struct Row_insert {
 ...
 private:
   ...
   */
  void foreign_report_add_err(Trx *trx, const Foreign *foreign, const Rec rec, const DTuple *entry) noexcept;
  ...
  * @brief Checks if a unique key violation would occur at the index entry insert.
   ...
   */
  [[nodiscard]] bool dupl_error_with_rec(const Rec rec, const DTuple *entry, const Index *index, const ulint *offsets) noexcept;
