...
inline trx_undo_rec_t *trx_undo_page_get_prev_rec(trx_undo_rec_t *rec, page_no_t page_no, ulint offset) {
  const auto undo_page = reinterpret_cast<page_t *>(ut_align_down(rec, UNIV_PAGE_SIZE));
  const auto start = trx_undo_page_get_start(undo_page, page_no, offset);

  return start + undo_page == rec ? nullptr : undo_page + mach_read_from_2(rec - 2);
}
...
inline trx_undo_rec_t *trx_undo_page_get_next_rec(trx_undo_rec_t *rec, page_no_t page_no, ulint offset) {
  auto undo_page = reinterpret_cast<page_t *>(ut_align_down(rec, UNIV_PAGE_SIZE));
  const auto end = trx_undo_page_get_end(undo_page, page_no, offset);
  const auto next = mach_read_from_2(rec);
...
inline trx_undo_rec_t *trx_undo_page_get_last_rec(page_t *undo_page, page_no_t page_no, ulint offset) {
  ...
  return start == end ? nullptr : undo_page + end - 2;
}
