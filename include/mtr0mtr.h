...
#define MLOG_REC_CLUST_DELETE_MARK                                             \
  ((byte)10) /*!< mark clustered index record                                  \
             deleted */
#define MLOG_REC_SEC_DELETE_MARK                                               \
  ((byte)11) /*!< mark secondary index record                                  \
             deleted */
#define MLOG_REC_UPDATE_IN_PLACE                                               \
  ((byte)13) /*!< update of a record,                                          \
             preserves record field sizes */
#define MLOG_REC_DELETE                                                        \
  ((byte)14) /*!< delete a record from a                                       \
             page */
#define MLOG_LIST_END_DELETE                                                   \
  ((byte)15) /*!< delete record list end on                                    \
             index page */
#define MLOG_LIST_START_DELETE                                                 \
  ((byte)16) /*!< delete record list start on                                  \
             index page */
#define MLOG_LIST_END_COPY_CREATED                                             \
  ((byte)17) /*!< copy record list end to a                                    \
             new created index page */
#define MLOG_PAGE_REORGANIZE                                                   \
  ((byte)18)                        /*!< reorganize an                         \
                                    index page in                              \
                                    ROW_FORMAT=REDUNDANT */
#define MLOG_PAGE_CREATE ((byte)19) /*!< create an index page */
#define MLOG_UNDO_INSERT                                                       \
  ((byte)20) /*!< insert entry in an undo                                      \
             log */
#define MLOG_UNDO_ERASE_END                                                    \
  ((byte)21) /*!< erase an undo log                                            \
             page end */
#define MLOG_UNDO_INIT                                                         \
  ((byte)22) /*!< initialize a page in an                                      \
             undo log */
#define MLOG_UNDO_HDR_DISCARD                                                  \
  ((byte)23) /*!< discard an update undo log                                   \
             header */
#define MLOG_UNDO_HDR_REUSE                                                    \
  ((byte)24) /*!< reuse an insert undo log                                     \
             header */
#define MLOG_UNDO_HDR_CREATE                                                   \
  ((byte)25) /*!< create an undo                                               \
             log header */
#define MLOG_REC_MIN_MARK                                                      \
  ((byte)26) /*!< mark an index                                                \
             record as the                                                     \
             predefined minimum                                                \
             record */
...
#define MLOG_WRITE_STRING                                                      \
  ((byte)30) /*!< write a string to                                            \
             a page */
#define MLOG_MULTI_REC_END                                                     \
  ((byte)31) /*!< if a single mtr writes                                       \
             several log records,                                              \
             this log record ends the                                          \
             sequence of these records */
#define MLOG_DUMMY_RECORD                                                      \
  ((byte)32) /*!< dummy log record used to                                     \
             pad a log block full */
#define MLOG_FILE_CREATE                                                       \
  ((byte)33) /*!< log record about an .ibd                                     \
             file creation */
#define MLOG_FILE_RENAME                                                       \
  ((byte)34) /*!< log record about an .ibd                                     \
             file rename */
#define MLOG_FILE_DELETE                                                       \
  ((byte)35) /*!< log record about an .ibd                                     \
             file deletion */
#define MLOG_COMP_REC_MIN_MARK                                                 \
  ((byte)36) /*!< mark a compact                                               \
             index record as the                                               \
             predefined minimum                                                \
             record */
#define MLOG_COMP_PAGE_CREATE                                                  \
  ((byte)37)                            /*!< create a compact                  \
                                        index page */
...
#define MLOG_COMP_REC_DELETE                                                   \
  ((byte)42) /*!< delete a compact record                                      \
             from a page */
#define MLOG_COMP_LIST_END_DELETE                                              \
  ((byte)43) /*!< delete compact record list                                   \
             end on index page */
#define MLOG_COMP_LIST_START_DELETE                                            \
  ((byte)44) /*!< delete compact record list                                   \
             start on index page */
...
#define MLOG_FILE_FLAG_TEMP                                                    \
  1 /*!< identifies TEMPORARY TABLE in                                         \
...
inline mtr_t *
mtr_start(mtr_t *mtr); /*!< in: memory buffer for the mtr buffer */
/** Commits a mini-transaction. */

void mtr_commit(mtr_t *mtr); /*!< in: mini-transaction */
/** Sets and returns a savepoint in mtr.
@return	savepoint */
inline ulint mtr_set_savepoint(mtr_t *mtr); /*!< in: mtr */
/** Releases the latches stored in an mtr memo down to a savepoint.
NOTE! The mtr must not have made changes to buffer pages after the
savepoint, as these can be handled only by mtr_commit. */

void mtr_rollback_to_savepoint(mtr_t *mtr,       /*!< in: mtr */
                               ulint savepoint); /*!< in: savepoint */
/** Releases the (index tree) s-latch stored in an mtr memo after a
savepoint. */
inline void
mtr_release_s_latch_at_savepoint(mtr_t *mtr,       /*!< in: mtr */
                                 ulint savepoint,  /*!< in: savepoint */
                                 rw_lock_t *lock); /*!< in: latch to release */
/** Gets the logging mode of a mini-transaction.
@return	logging mode: MTR_LOG_NONE, ... */
inline ulint mtr_get_log_mode(mtr_t *mtr); /*!< in: mtr */
/** Changes the logging mode of a mini-transaction.
@return	old mode */
inline ulint
mtr_set_log_mode(mtr_t *mtr,  /*!< in: mtr */
...
void mtr_memo_release(mtr_t *mtr,   /*!< in: mtr */
                      void *object, /*!< in: object */
                      ulint type); /*!< in: object type: MTR_MEMO_S_LOCK, ... */
#ifdef UNIV_DEBUG
/** Checks if memo contains the given item.
@return	true if contains */
inline bool mtr_memo_contains(mtr_t *mtr,         /*!< in: mtr */
...
inline void
mtr_memo_push(mtr_t *mtr,   /*!< in: mtr */
              void *object, /*!< in: object */
              ulint type);  /*!< in: object type: MTR_MEMO_S_LOCK, ... */
...
struct mtr_struct {
  ...
  uint64_t start_lsn; /* start lsn of the possible log entry for
                       this mtr */
  uint64_t end_lsn;   /* end lsn of the possible log entry for
                         this mtr */
#ifdef UNIV_DEBUG
  ulint magic_n;
#endif /* UNIV_DEBUG */
