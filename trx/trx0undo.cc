...
static void
trx_undo_page_init(page_t *undo_page, /*!< in: undo log segment page */
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  prev_page = trx_undo_page_get_s_latched(space, zip_size, prev_page_no, mtr);
...
trx_undo_rec_t *
trx_undo_get_prev_rec(trx_undo_rec_t *rec, /*!< in: undo record */
                      ulint page_no, /*!< in: undo log header page number */
                      ulint offset,  /*!< in: undo log header offset on page */
                      mtr_t *mtr)    /*!< in: mtr */
{
    ...
    mtr_t *mtr)        /*!< in: mtr */
{
  ...
  if (mode == RW_S_LATCH) {
    next_page = trx_undo_page_get_s_latched(space, zip_size, next_page_no, mtr);
  } else {
    ut_ad(mode == RW_X_LATCH);
    next_page = trx_undo_page_get(space, zip_size, next_page_no, mtr);
  }
...
trx_undo_rec_t *
trx_undo_get_next_rec(trx_undo_rec_t *rec, /*!< in: undo record */
                      ulint page_no, /*!< in: undo log header page number */
                      ulint offset,  /*!< in: undo log header offset on page */
                      mtr_t *mtr)    /*!< in: mtr */
{
  ...
  return (trx_undo_get_next_rec_from_next_page(
      space, zip_size, page_align(rec), page_no, offset, RW_S_LATCH, mtr));
}
    ...
    mtr_t *mtr)     /*!< in: mtr */
{
  ...
  return (trx_undo_get_next_rec_from_next_page(space, zip_size, undo_page,
...
byte *trx_undo_parse_page_init(byte *ptr,     /*!< in: buffer */
...
trx_undo_seg_create(trx_rseg_t *rseg
                    __attribute__((unused)), /*!< in: rollback segment */
...
static ulint
trx_undo_header_create(page_t *undo_page, /*!< in/out: undo log segment
...
{
#ifdef WITH_XOPEN
  mlog_write_ulint(log_hdr + TRX_UNDO_XA_FORMAT, (ulint)xid->formatID,
                   MLOG_4BYTES, mtr);

  mlog_write_ulint(log_hdr + TRX_UNDO_XA_TRID_LEN, (ulint)xid->gtrid_length,
                   MLOG_4BYTES, mtr);

  mlog_write_ulint(log_hdr + TRX_UNDO_XA_BQUAL_LEN, (ulint)xid->bqual_length,
                   MLOG_4BYTES, mtr);
  ...
  mlog_write_string(log_hdr + TRX_UNDO_XA_XID, (const byte *)buf, XIDDATASIZE,
                    mtr);
#endif /* WITH_XOPEN */
}
...
static void
trx_undo_read_xid(trx_ulogf_t *log_hdr, /*!< in: undo log header */
...
UNIV_INLINE
void trx_undo_discard_latest_log(
...
byte *
trx_undo_parse_discard_latest(byte *ptr, /*!< in: buffer */
                              byte *end_ptr
...
ulint trx_undo_add_page(trx_t *trx, trx_undo_t *undo, mtr_t *mtr) {
  ...
  header_page = trx_undo_page_get(undo->space, undo->zip_size, undo->hdr_page_no, mtr);
  ...
  page_no = fseg_alloc_free_page_general(
      header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_FSEG_HEADER,
      undo->top_page_no + 1, FSP_UP, TRUE, mtr);
  ...
  new_page = trx_undo_page_get(undo->space, undo->zip_size, page_no, mtr);
...
static ulint
trx_undo_free_page(trx_rseg_t *rseg,  /*!< in: rollback segment */
                   ibool in_history,  /*!< in: TRUE if the undo log is in the
                                      history  list */
                               ...
                               the rollback segment mutex */
{
  ...
  if (in_history) {
    rseg_header = trx_rsegf_get(space, zip_size, rseg->page_no, mtr);
...
static void
trx_undo_empty_header_page(ulint space,       /*!< in: space */
                      ...
                      >= this value should be truncated */
{
  ...
  for (;;) {
    ...
    undo_page =
        trx_undo_page_get(undo->space, undo->zip_size, last_page_no, &mtr);
...
loop:
  ...
  rec = trx_undo_get_first_rec(space, rseg->zip_size, hdr_page_no, hdr_offset, RW_X_LATCH, &mtr);
  ...
  if (page_no == hdr_page_no) {
    trx_undo_empty_header_page(space, rseg->zip_size, hdr_page_no, hdr_offset,
                               &mtr);
  } else {
...
static void trx_undo_seg_free(trx_undo_t *undo) /*!< in: undo log */
{
  ...
  do {
    ...
    seg_header = trx_undo_page_get(undo->space, undo->zip_size, undo->hdr_page_no, &mtr) + TRX_UNDO_SEG_HDR;
    ...
    if (finished) {
      ...
      rseg_header =
          trx_rsegf_get(rseg->space, rseg->zip_size, rseg->page_no, &mtr);
    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  undo_page = trx_undo_page_get(rseg->space, rseg->zip_size, page_no, mtr);
  ...
  last_page =
      trx_undo_page_get(rseg->space, rseg->zip_size, undo->last_page_no, mtr);
...
ulint trx_undo_lists_init( ib_recovery_t recovery, trx_rseg_t *rseg) {
  ...
  rseg_header =
      trx_rsegf_get_new(rseg->space, rseg->zip_size, rseg->page_no, &mtr);
...
static db_err 
trx_undo_create(trx_t *trx,        /*!< in: transaction */
    ...
    mtr_t *mtr)     /*!< in: mtr */
{
  ...
  undo_page =
      trx_undo_page_get(undo->space, undo->zip_size, undo->hdr_page_no, mtr);
...
transaction. */
static void
trx_undo_mark_as_dict_operation(trx_t *trx, /*!< in: dict op transaction */
                                trx_undo_t *undo, /*!< in: assigned undo log */
                                mtr_t *mtr)       /*!< in: mtr */
{
  page_t *hdr_page;

  hdr_page = trx_undo_page_get(undo->space, undo->zip_size, undo->hdr_page_no, mtr);
  ...
  mlog_write_dulint(hdr_page + undo->hdr_offset + TRX_UNDO_TABLE_ID, undo->table_id, mtr);
...
page_t *trx_undo_set_state_at_finish(trx_rseg_t *rseg, trx_t *trx __attribute__((unused)), trx_undo_t *undo, mtr_t *mtr) {
  ...
  undo_page = trx_undo_page_get(undo->space, undo->zip_size, undo->hdr_page_no, mtr);
