...
static const char *file_format_name_map[] = {
    "Antelope", "Barracuda", "Cheetah", "Dragon",   "Elk",      "Fox",
    "Gazelle",  "Hornet",    "Impala",  "Jaguar",   "Kangaroo", "Leopard",
    "Moose",    "Nautilus",  "Ocelot",  "Porpoise", "Quail",    "Rabbit",
    "Shark",    "Tiger",     "Urchin",  "Viper",    "Whale",    "Xenops",
    "Yak",      "Zebra"};
...
ibool trx_doublewrite_page_inside(ulint page_no) /*!< in: page number */
{
...
enum db_err trx_sys_create_doublewrite_buf(void) {
  buf_block_t *block;
  buf_block_t *block2;
#ifdef UNIV_DEBUG
...
start_again:
  ...
  if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC) ==
    ...
    fseg_header = buf_block_get_frame(block) + TRX_SYS_DOUBLEWRITE +
    ...
    for (i = 0; i < 2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2;
      ...
      if (i == FSP_EXTENT_SIZE / 2) {
        ...
        mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK1, page_no,
                         MLOG_4BYTES, &mtr);
        mlog_write_ulint(doublewrite + TRX_SYS_DOUBLEWRITE_REPEAT +
    ...
    ibool restore_corrupt_pages) /*!< in: TRUE=restore pages */
{
  ...
  if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED) !=
    ...
    ib_logger(ib_stream, "InnoDB: Resetting space id's in the"
                         " doublewrite buffer\n");
  } else {
    trx_sys_multiple_tablespace_format = TRUE;
  }
  ...
  for (i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++) {
    ...
    if (!restore_corrupt_pages) {
      ...
      ib_logger(ib_stream,
               ...
               ((page_no >= block1 &&
                 page_no < block1 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE) ||
                (page_no >= block2 &&
                 page_no < (block2 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE)))) {
...
#ifndef WITH_ZIP
      if (zip_size > 0) {
        srv_panic(DB_UNSUPPORTED, "InnoDB: Warning: attempt to "
      ...
      if (UNIV_UNLIKELY(buf_page_is_corrupted(read_buf, zip_size))) {
        ...
        if (buf_page_is_corrupted(page, zip_size)) {
          ...
          ib_logger(ib_stream, "InnoDB: Also the page in the"
                               ...
                               "InnoDB: "
                               "force_recovery=6\n");
          srv_panic(DB_CORRUPTION, "Corrupt page");
        }
        ...
        ib_logger(ib_stream, "InnoDB: Recovered the page from"
                             " the doublewrite buffer.\n");
      }
    }
...
static void trx_sysf_create(mtr_t *mtr) /*!< in: mtr */
{
  ...
  UNIV_MEM_VALID(sys_header +
                     (TRX_SYS_RSEGS + TRX_SYS_N_RSEGS * TRX_SYS_RSEG_SLOT_SIZE +
                      TRX_SYS_RSEG_SPACE),
...
void trx_sys_init_at_db_start(ib_recovery_t recovery) /*!< in: recovery flag */
{
  ...
  if (UT_LIST_GET_LEN(trx_sys->trx_list) > 0) {
    ...
    ib_logger(ib_stream, "InnoDB: Trx id counter is " TRX_ID_FMT "\n",
...
const char *
trx_sys_file_format_id_to_name(const ulint id) /*!< in: id of the file format */
{
...
const char *trx_sys_file_format_max_get(void) { return (file_format_max.name); }
