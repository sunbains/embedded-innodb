...
que_thr_t *trx_commit_step(que_thr_t *thr) /*!< in: query thread */
{
  ...
  if (node->state == COMMIT_NODE_SEND) {
    ...
    trx_sig_send(thr_get_trx(thr), TRX_SIG_COMMIT, TRX_SIG_SELF, thr, NULL,
                 &next_thr);
...
ulint trx_commit_flush_log(trx_t *trx) /*!< in: trx handle */
{
                                      ...
                                      0 to use the default max length */
{
  ...
  ib_logger(ib_stream, "TRANSACTION " TRX_ID_FMT, TRX_ID_PREP_PRINTF(trx->id));
...
void trx_prepare_off_kernel(trx_t *trx) /*!< in: transaction */
{
  trx_rseg_t *rseg;
  ib_uint64_t lsn = 0;
  mtr_t mtr;
