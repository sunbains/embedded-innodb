...
void Undo_page::add_undo_rec_log(ulint old_free, ulint new_free, mtr_t *mtr) noexcept {
  ...
  log_ptr = mlog_write_initial_log_record_fast(*this, MLOG_UNDO_INSERT, log_ptr, mtr);
  ...
  if (log_ptr + len <= log_end) {
    memcpy(log_ptr, *this + old_free + 2, len);
    mlog_close(mtr, log_ptr + len);
  } else {
    mlog_close(mtr, log_ptr);
    mlog_catenate_string(mtr, *this + old_free + 2, len);
  }
}

byte *trx_undo_parse_add_undo_rec(byte *ptr, byte *end_ptr, Raw_page page) {
  ulint len;
  byte *rec;
  ulint first_free;
 ...
 */
inline ulint trx_undo_left(const Raw_page page, const byte *ptr) noexcept {
...
ulint Undo_page::set_next_prev_and_add(byte *ptr, mtr_t *mtr) noexcept {
  ut_ad(ptr > *this);
  ut_ad(ptr < *this + UNIV_PAGE_SIZE);

  if (unlikely(trx_undo_left(*this, ptr) < 2)) {

    return 0;
  }

  /* pointer within undo_page that points to the next free offset value within undo_page.*/
  auto ptr_to_first_free = *this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_FREE;
  ...
  auto end_of_rec = ptr - *this;

  /* Write offset of the next undo log record */
  mach_write_to_2(*this + first_free, end_of_rec);

  /* Update the offset to first free undo record */
  mach_write_to_2(ptr_to_first_free, end_of_rec);

  /* Write this log entry to the UNDO log */
  add_undo_rec_log(first_free, end_of_rec, mtr);

  return first_free;
}

ulint Undo_page::report_insert(Trx *trx, const Index *index, const DTuple *clust_entry, mtr_t *mtr) {
  ulint first_free;
  byte *ptr;
  ulint i;

  ut_ad(index->is_clustered());
  ut_ad(mach_read_from_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_TYPE) == TRX_UNDO_INSERT);

  first_free = mach_read_from_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_FREE);
  ptr = *this + first_free;

  ut_ad(first_free <= UNIV_PAGE_SIZE);

  if (trx_undo_left(*this, ptr) < 2 + 1 + 11 + 11) {
  ...
  *ptr = TRX_UNDO_INSERT_REC;
  ...
  for (i = 0; i < index->get_n_unique(); i++) {
    ...
    if (flen != UNIV_SQL_NULL) {
      if (trx_undo_left(*this, ptr) < flen) {
  ...
  return set_next_prev_and_add(ptr, mtr);
}
...
ulint Undo_page::report_modify(
  ...
  ut_ad(mach_read_from_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_TYPE) == TRX_UNDO_UPDATE);
  auto table = index->m_table;

  auto first_free = mach_read_from_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_FREE);
  ptr = *this + first_free;

  ut_ad(first_free <= UNIV_PAGE_SIZE);

  if (trx_undo_left(*this, ptr) < 50) {
  ...
  *ptr++ = (byte)rec.get_info_bits();

  /* Store the values of the system columns */
  field = rec.get_nth_field(offsets, index->get_sys_col_field_pos(DATA_TRX_ID), &flen);
  ...
  field = rec.get_nth_field(offsets, index->get_sys_col_field_pos(DATA_ROLL_PTR), &flen);
  ...
  for (ulint i = 0; i < index->get_n_unique(); i++) {
    ...
    field = rec.get_nth_field(offsets, i, &flen);
  ...
  if (update) {
    ...
    for (ulint i = 0; i < Row_update::upd_get_n_fields(update); i++) {
      ...
      field = rec.get_nth_field(offsets, pos, &flen);

      if (trx_undo_left(*this, ptr) < 15) {
  ...
  if (!update || !(cmpl_info & UPD_NODE_NO_ORD_CHANGE)) {
    ...
    for (ulint col_no = 0; col_no < table->get_n_cols(); col_no++) {
      ...
      if (col->m_ord_part) {
        /* Write field number to undo log */
        if (trx_undo_left(*this, ptr) < 5 + 15) {
  ...
  mach_write_to_2(ptr, first_free);
  ptr += 2;
  mach_write_to_2(*this + first_free, ptr - *this);

  mach_write_to_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_FREE, ptr - *this);

  /* Write to the REDO log about this change in the UNDO log */

  add_undo_rec_log(first_free, ptr - *this, mtr);
  return first_free;
}
...
void Undo_page::erase_page_end(mtr_t *mtr) {
  auto first_free = mach_read_from_2(*this + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_FREE);

  memset(*this + first_free, 0xff, (UNIV_PAGE_SIZE - FIL_PAGE_DATA_END) - first_free);

  mlog_write_initial_log_record(*this, MLOG_UNDO_ERASE_END, mtr);
}

byte *trx_undo_parse_erase_page_end(byte *ptr, byte *end_ptr __attribute__((unused)), Raw_page page, mtr_t *mtr) {
  ut_ad(ptr && end_ptr);
  ...
  Undo_page(page).erase_page_end(mtr);
...
db_err trx_undo_report_row_operation(
  ...
  for (;;) {
    ...
    Undo_page undo_page(undo_block->get_frame());

    ulint offset;

    if (op_type == TRX_UNDO_INSERT_OP) {
      offset = undo_page.report_insert(trx, index, clust_entry, &mtr);
    } else {
      offset = undo_page.report_modify(trx, index, rec, offsets, update, cmpl_info, &mtr);
    }
