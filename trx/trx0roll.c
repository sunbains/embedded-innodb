...
static void
trx_rollback_active(ib_recovery_t recovery, /*!< in: recovery flag */
...
static void
trx_undo_arr_remove_info(trx_undo_arr_t *arr, /*!< in: undo number array */
                         undo_no_t undo_no)   /*!< in: undo number */
{
  trx_undo_inf_t *cell;
  ulint i;
...
trx_roll_pop_top_rec(trx_t *trx,       /*!< in: transaction */
...
void trx_rollback(trx_t *trx,           /*!< in: transaction */
                  ...
                  que_thr_t **next_thr) /*!< in/out: next query thread to run;
                                       ...
                                       a new query thread; if the passed value
...
que_thr_t *trx_rollback_step(que_thr_t *thr) /*!< in: query thread */
{
  ...
  if (node->state == ROLL_NODE_SEND) {
    ...
    trx_sig_send(thr_get_trx(thr), sig_no, TRX_SIG_SELF, thr, savept, NULL);
