                             ...
                             int page_size)      /*!< in: page size */
{
  ...
  err =
      ib_table_schema_add_col(ib_tbl_sch, "c2", IB_INT, 0, 0, sizeof(ib_i32_t));

  assert(err == DB_SUCCESS);

  err =
      ib_table_schema_add_col(ib_tbl_sch, "c3", IB_FLOAT, 0, 0, sizeof(float));

  assert(err == DB_SUCCESS);

  err = ib_table_schema_add_col(ib_tbl_sch, "c4", IB_DOUBLE, 0, 0,
                                sizeof(double));
