...
private:
  ...
  static void test_reserve_and_write_fast() {
    ...
    lsn_t start_lsn2;
    lsn_t end_lsn2 = log.reserve_and_write_fast(large_data.c_str(), large_data.length(), &start_lsn2);
...
void manual_log_test() {
  ...
  const std::vector<std::string> test_records = {
    "First log record",
    "Second log record with more data",
    "Third record",
  ...
  for (const auto& record : test_records) {
    lsn_t start_lsn;
    lsn_t end_lsn = log.reserve_and_write_fast(record.c_str(), record.length(), &start_lsn);
    
    if (end_lsn != 0) {
      std::cout << "Wrote record: \"" << record << "\" from LSN " << start_lsn << " to " << end_lsn << "\n";
    } else {
      std::cout << "Failed to write record: \"" << record << "\" (not enough space in block)\n";
    }
  }
