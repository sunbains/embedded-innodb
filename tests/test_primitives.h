...
inline void test_event_set(Cond_var* event) {
  os_event_set(event);
}

inline void test_event_free(Cond_var* event) {
  os_event_free(event);
}

// Test memory allocator wrappers - can be instrumented for leak detection
inline void* test_mem_alloc(ulint size) {
  return mem_alloc(size);
}

inline void test_mem_free(void* ptr) {
  mem_free(ptr);
}