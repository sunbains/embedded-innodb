...
byte *
page_mem_alloc_heap(page_t *page,             /*!< in/out: index page */
...
byte *page_parse_create(byte *ptr, /*!< in: buffer */
                        byte *end_ptr
...
#error "PAGE_BTR_IBUF_FREE_LIST_NODE + FLST_NODE_SIZE > PAGE_DATA"
  ...
  page_header_set_field(page, NULL, PAGE_N_HEAP,
...
page_t *page_create(buf_block_t *block, /*!< in: a buffer block where the
                                        page is created */
                    mtr_t *mtr,         /*!< in: mini-transaction handle */
                    ulint comp)         /*!< in: nonzero=compact page format */
{
...
page_t *page_create_zip(buf_block_t *block,  /*!< in/out: a buffer frame where
                                             the  page is created */
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  if (likely_null(new_page_zip)) {
    ...
    if (unlikely(!page_zip_compress(new_page_zip, new_page, index, mtr))) {
      ...
      if (unlikely(!page_zip_reorganize(new_block, index, mtr))) {

        if (unlikely(!page_zip_decompress(new_page_zip, new_page, false))) {
          ut_error;
        }
...
bool page_simple_validate_new(page_t *page) /*!< in: new-style index page */
{
  page_dir_slot_t *slot;
  ulint slot_no;
  ulint n_slots;
