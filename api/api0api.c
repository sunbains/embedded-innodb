	...
	ib_tuple_t*	tuple)		/*!< in: tuple to read into */
{
	...
	offsets = rec_get_offsets(
		rec, dindex, offsets, ULINT_UNDEFINED, &tuple->heap);
	...
	*dict_index = row_merge_create_index(ddl_trx, table, index_def);
	...
	if (*dict_index != NULL) {
		...
		err = row_merge_build_indexes(
			usr_trx, table, table, dict_index, 1, NULL);
	}
	...
	mem_heap_t*	heap)		/*!< in: memory heap to use */
{
	...
	for (i = 0; i < n_index_defs; ++i) {
		...
		dict_index = row_merge_create_index(trx, new_table,
						&index_defs[i]);
	...
	if (n_indexes > 0) {
		...
		if (ib_clust_index_def != NULL) {
			...
			err = ib_create_cluster_index(
				ddl_trx, table, ib_clust_index_def,
				&dict_index);
		}
	}

	if (ib_clust_index_def == NULL) {
		err = ib_create_default_cluster_index(ddl_trx, table,
						&dict_index);
	}
	...
	for (i= 0; err == DB_SUCCESS && i < n_indexes; ++i) {
		...
		if (!ib_index_def->clustered) {
			...
			err = ib_build_secondary_index(
				ddl_trx, table, ib_index_def, IB_TRUE,
				&dict_index);
		} else {
	...
	ib_id_t*	index_id)	/*!< out: index id */
{
	...
	if (err == DB_SUCCESS) {
		...
		to IB_FALSE. */
		err = ib_build_secondary_index(
			usr_trx, table, ib_index_def, IB_FALSE, &dict_index);
	...
	ib_id_t		index_id)	/*!< in: index id to drop */
{
	...
	dict_index = dict_index_get_on_id_low(
