  ...
  for (i = 0; i < n_fields; i++) {
    ...
    if (dfield_is_null(new_val) && !rec_offs_comp(offsets)) {
      ...
      new_len = dict_col_get_sql_null_size(
          dict_index_get_nth_col(index, upd_field->field_no), 0);
    }
    ...
    mtr_t *mtr __attribute__((unused))) /*!< in: mtr */
{
  ...
  log_ptr += mach_write_compressed(
      log_ptr, dict_index_get_sys_col_pos(index, DATA_TRX_ID));
...
byte *row_upd_parse_sys_vals(byte *ptr,     /*!< in: buffer */
...
#error "MLOG_BUF_MARGIN <= 30"
    ...
    if (len != UNIV_SQL_NULL) {
      if (log_ptr + len < buf_end) {
        ...
        mlog_catenate_string(mtr, dfield_get_data(new_val), len);
...
byte *row_upd_index_parse(byte *ptr,          /*!< in: buffer */
                          byte *end_ptr,      /*!< in: buffer end */
...
@return	BLOB prefix */
static byte *
row_upd_ext_fetch(const byte *data, /*!< in: 'internally' stored part of the
...
void row_upd_replace(dtuple_t *row,   /*!< in/out: row where replaced,
                                      indexed by col_no;
                                      the clustered index record must be
                                      covered by a lock or a page latch to
                                      prevent deletion (rollback or purge) */
  ...
  for (i = 0; i < n_unique; i++) {
    ...
    for (j = 0; j < n_upd_fields; j++) {
      ...
      if (col_pos == upd_field->field_no &&
          (row == NULL || ind_field->prefix_len > 0 ||
...
static void row_upd_store_row(upd_node_t *node) /*!< in: row update node */
{
  ...
  node->row = row_build(ROW_COPY_DATA, clust_index, rec, offsets, NULL,
                        &node->ext, node->heap);
  if (node->is_delete) {
    node->upd_row = NULL;
    node->upd_ext = NULL;
...
code or DB_LOCK_WAIT */
UNIV_INLINE
ulint row_upd_sec_step(upd_node_t *node, /*!< in: row update node */
                       que_thr_t *thr)   /*!< in: query thread */
{
...
@return DB_SUCCESS if operation successfully completed, else error
code or DB_LOCK_WAIT */
static ulint row_upd_clust_rec_by_insert(
    upd_node_t *node,    /*!< in: row update node */
...
@return DB_SUCCESS if operation successfully completed, else error
code or DB_LOCK_WAIT */
static ulint row_upd_clust_rec(upd_node_t *node,    /*!< in: row update node */
                               dict_index_t *index, /*!< in: clustered index */
                               que_thr_t *thr,      /*!< in: query thread */
                               mtr_t *mtr) /*!< in: mtr; gets committed here */
{
...
static ulint row_upd_del_mark_clust_rec(
    upd_node_t *node,    /*!< in: row update node */
    dict_index_t *index, /*!< in: clustered index */
...
static ulint row_upd_clust_step(upd_node_t *node, /*!< in: row update node */
                                que_thr_t *thr)   /*!< in: query thread */
{
  dict_index_t *index;
  btr_pcur_t *pcur;
  ...
  if (!node->has_clust_rec_x_lock) {
    err = lock_clust_rec_modify_check_and_lock(0, btr_pcur_get_block(pcur), rec,
                                               index, offsets, thr);
    if (err != DB_SUCCESS) {
      mtr_commit(mtr);
      goto exit_func;
...
@return DB_SUCCESS if operation successfully completed, else error
code or DB_LOCK_WAIT */
static ulint row_upd(upd_node_t *node, /*!< in: row update node */
                     que_thr_t *thr)   /*!< in: query thread */
{
  ulint err = DB_SUCCESS;
