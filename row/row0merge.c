    ...
    ulint buf_size)      /*!< in: size of the buffer, in bytes */
{
  ...
  buf->tuples = mem_heap_alloc(heap, 2 * max_tuples * sizeof *buf->tuples);
...
static ibool row_merge_read(int fd,       /*!< in: file descriptor */
                            ...
                            row_merge_block_t *buf) /*!< out: data */
{
  ...
  if (UNIV_UNLIKELY(!success)) {
    ...
    ib_logger(ib_stream, "  InnoDB: failed to read merge block at %llu\n", ofs);
  }
...
/** Read a merge block from the file system.
@return	TRUE if request was successful, FALSE if fail */
static ibool row_merge_write(int fd,          /*!< in: file descriptor */
                             ulint offset,    /*!< in: offset where to write */
                             const void *buf) /*!< in: data */
{
  ib_uint64_t ofs = ((ib_uint64_t)offset) * sizeof(row_merge_block_t);
  ...
  return (UNIV_LIKELY(os_file_write(
      "(merge)", OS_FILE_FROM_FD(fd), buf, (ulint)(ofs & 0xFFFFFFFF),
      (ulint)(ofs >> 32), sizeof(row_merge_block_t))));
}
...
static __attribute__((nonnull)) ulint row_merge_read_clustered_index(
    ...
    row_merge_block_t *block)      /*!< in/out: file buffer */
{
  ...
  for (;;) {
    ...
    for (i = 0; i < n_index; i++) {
      ...
      if (UNIV_LIKELY(row && row_merge_buf_add(buf, row, ext))) {
        file->n_rec++;
        continue;
      }
      ...
      if (UNIV_LIKELY(row != NULL)) {
        ...
        if (UNIV_UNLIKELY(!row_merge_buf_add(buf, row, ext))) {
                                            ...
                                            if applicable */
{
  ...
  while (mrec0 && mrec1) {
    ...
    case 0:
      if (UNIV_UNLIKELY(dict_index_is_unique(index))) {
        mem_heap_free(heap);
...
static __attribute__((nonnull)) ibool
...
/** Merge disk files.
@return	DB_SUCCESS or error code */
static __attribute__((nonnull)) ulint
                                     ...
                                     if applicable */
{
  ...
  for (; foffs0 < ihalf && foffs1 < file->offset; foffs0++, foffs1++) {
    ...
    error = row_merge_blocks(index, file, block, &foffs0, &foffs1, &of, table);
                                          ...
                                          if applicable */
{
  ...
  do {
    ...
    error = row_merge(trx, index, file, &half, block, tmpfd, table);
...
static ulint row_merge_insert_index_tuples(
    trx_t *trx,               /*!< in: transaction */
...
ulint row_merge_rename_indexes(
    trx_t *trx,          /*!< in/out: transaction */
...
ulint row_merge_drop_table(trx_t *trx,          /*!< in: transaction */
                             ...
                             if applicable */
{
  ...
  error = row_merge_read_clustered_index(
      trx, table, old_table, new_table, indexes, merge_files, n_indexes, block);
  ...
  for (i = 0; i < n_indexes; i++) {
    ...
    if (error == DB_SUCCESS) {
      error = row_merge_insert_index_tuples(trx, indexes[i], new_table,
