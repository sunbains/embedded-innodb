    ...
    ins_node_t *node) /*!< in: row insert node */
{
  ...
  while (index != NULL) {
    entry = row_build_index_entry(node->row, NULL, index, node->entry_sys_heap);
...
static ulint row_ins_sec_index_entry_by_modify(
...
static ulint row_ins_clust_index_entry_by_modify(
    ulint mode,            /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE,
                           depending on whether mtr holds just a leaf
                           latch or also a tree latch */
    btr_cur_t *cursor,     /*!< in: B-tree cursor */
      ...
      if (parent_ufield->field_no == parent_field_no) {
        ...
        if (!dfield_is_null(&ufield->new_val) &&
            dtype_get_at_most_n_mbchars(
                ...
                dfield_get_data(&ufield->new_val)) < ufield_len) {
        ...
        if (min_size > ufield_len) {
          ...
          char *pad_start;
          const char *pad_end;
          char *padded_data = mem_heap_alloc(heap, min_size);
          ...
          default:
            ...
            return (ULINT_UNDEFINED);
          case 1:
            if (UNIV_UNLIKELY(dtype_get_charset_coll(col->prtype) ==
...
static ulint row_update_cascade_client(
    que_thr_t *thr,      /*!< in: query thread */
...
static ulint row_ins_foreign_check_on_constraint(
    que_thr_t *thr,          /*!< in: query thread whose run_node
                             is an update node */
    dict_foreign_t *foreign, /*!< in: foreign key constraint whose
                             type is != 0 */
...
@return	DB_SUCCESS or error code */
static ulint row_ins_set_shared_rec_lock(
...
records
@return	DB_SUCCESS or error code */
static ulint row_ins_set_exclusive_rec_lock(
    ulint type,               /*!< in: LOCK_ORDINARY, LOCK_GAP, or
                              LOCK_REC_NOT_GAP type lock */
    const buf_block_t *block, /*!< in: buffer block of rec */
    const rec_t *rec,         /*!< in: record */
...
a constraint.
@return	DB_SUCCESS or error code */
static ulint row_ins_check_foreign_constraints(
    dict_table_t *table, /*!< in: table */
    dict_index_t *index, /*!< in: index */
    dtuple_t *entry,     /*!< in: index entry for index */
    que_thr_t *thr)      /*!< in: query thread */
...
or error code */
static ulint row_ins_index_entry_low(
    ulint mode,          /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE,
