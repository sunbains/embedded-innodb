...
part_loop:
  ...
  log_block = static_cast<byte*>(ut_align_down(log->buf + log->buf_free, OS_FILE_LOG_BLOCK_SIZE));
...
static void
log_pad_current_log_block(ib_recovery_t recovery) /*!< in: recovery flag */
{
...
ulint log_group_get_capacity(const log_group_t *group) /*!< in: log group */
{
  ut_ad(mutex_own(&(log_sys->mutex)));
...
UNIV_INLINE
ulint log_group_calc_size_offset(ulint offset, /*!< in: real offset within the
                                               log group */
...
UNIV_INLINE
ulint log_group_calc_real_offset(ulint offset, /*!< in: size offset within the
                                               log group */
                                 const log_group_t *group) /*!< in: log group */
{
  ut_ad(mutex_own(&(log_sys->mutex)));
...
static ibool log_calc_max_ages(void) {
  log_group_t *group;
  ulint margin;
  ulint free;
  ibool success = TRUE;
  ...
  log_sys->max_archived_lsn_age_async =
...
void innobase_log_init(void) {
  ...
  log_sys->buf_ptr = static_cast<byte*>(mem_alloc(LOG_BUFFER_SIZE + OS_FILE_LOG_BLOCK_SIZE));
  log_sys->buf = static_cast<byte*>(ut_align(log_sys->buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
  ...
  log_sys->checkpoint_buf_ptr = static_cast<byte*>(mem_alloc(2 * OS_FILE_LOG_BLOCK_SIZE));

  log_sys->checkpoint_buf =
      static_cast<byte*>(ut_align(log_sys->checkpoint_buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
...
used */
{
...
#ifdef UNIV_LOG_ARCHIVE
  group->archive_file_header_bufs_ptr = static_cast<byte*>(mem_alloc(sizeof(byte *) * n_files));

  group->archive_file_header_bufs = static_cast<byte*>(mem_alloc(sizeof(byte *) * n_files));
#endif /* UNIV_LOG_ARCHIVE */

  for (i = 0; i < n_files; i++) {
    group->file_header_bufs_ptr[i] =
        static_cast<byte*>(mem_alloc(LOG_FILE_HDR_SIZE + OS_FILE_LOG_BLOCK_SIZE));

    group->file_header_bufs[i] =
        static_cast<byte*>(ut_align(group->file_header_bufs_ptr[i], OS_FILE_LOG_BLOCK_SIZE));
  ...
  group->checkpoint_buf_ptr = static_cast<byte*>(mem_alloc(2 * OS_FILE_LOG_BLOCK_SIZE));

  group->checkpoint_buf =
      static_cast<byte*>(ut_align(group->checkpoint_buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
...
UNIV_INLINE
ulint log_group_check_flush_completion(log_group_t *group) /*!< in: log group */
{
  ut_ad(mutex_own(&(log_sys->mutex)));
...
static ulint log_sys_check_flush_completion(void) {
  ...
  if (log_sys->n_pending_writes == 0) {
    ...
    if (log_sys->write_end_offset > log_sys->max_buf_free / 2) {
      ...
      memmove(log_sys->buf, log_sys->buf + move_start,
                 move_end - move_start);
...
void log_group_read_log_seg(ulint type, /*!< in: LOG_ARCHIVE or LOG_RECOVER */
...
void log_archived_file_name_gen(char *buf, /*!< in: buffer where to write */
...
/** Writes a log file header to a log file space. */
static void log_group_archive_file_header_write(
    log_group_t *group,    /*!< in: log group */
    ulint nth_file,        /*!< in: header to the nth file in the
                           archive log file space */
    ulint file_no,         /*!< in: archived file number */
    uint64_t start_lsn) /*!< in: log file data starts at this
                           lsn */
{
  byte *buf;
  ulint dest_offset;
...
static void log_group_archive_completed_header_write(
    log_group_t *group,  /*!< in: log group */
    ulint nth_file,      /*!< in: header to the nth file in the
                         archive log file space */
    uint64_t end_lsn) /*!< in: end lsn of the file */
{
  byte *buf;
  ulint dest_offset;
...
static void log_group_archive(log_group_t *group) /*!< in: log group */
{
...
loop:
  if ((next_offset % group->file_size == 0) ||
    ...
    file_handle =
        os_file_create(name, open_mode, OS_FILE_AIO, OS_DATA_FILE, &ret);

    if (!ret && (open_mode == OS_FILE_CREATE)) {
      file_handle =
          os_file_create(name, OS_FILE_OPEN, OS_FILE_AIO, OS_DATA_FILE, &ret);
    }
    ...
    if (next_offset % group->file_size == 0) {
      log_group_archive_file_header_write(
          group, n_files, group->archived_file_no + n_files, start_lsn);
  ...
  if (log_debug_writes) {
    ib_logger(ib_stream,
              "Archiving starting at lsn %lu, len %lu"
              " to group %lu\n",
              start_lsn, (ulong)len, (ulong)group->id);
  }
#endif /* UNIV_DEBUG */
...
group is archived. */
static void log_archive_groups(void) {
  log_group_t *group;
...
the first log group. */
static void log_archive_write_complete_groups(void) {
  log_group_t *group;
  ulint end_offset;
  ulint trunc_files;
  ulint n_files;
  ...
  n_files = (UNIV_PAGE_SIZE * fil_space_get_size(group->archive_space_id)) /
            group->file_size;
  ut_ad(n_files > 0);
  ...
  start_lsn = log_sys->next_archived_lsn -
              (end_offset - LOG_FILE_HDR_SIZE +
               trunc_files * (group->file_size - LOG_FILE_HDR_SIZE));
  end_lsn = start_lsn;
...
#ifdef UNIV_DEBUG
  if (log_debug_writes) {
    ib_logger(ib_stream, "Archiving writes completed\n");
  }
#endif /* UNIV_DEBUG */
}

/** Completes an archiving i/o. */
static void log_archive_check_completion_low(void) {
  ut_ad(mutex_own(&(log_sys->mutex)));
...
#ifdef UNIV_DEBUG
    if (log_debug_writes) {
      ib_logger(ib_stream, "Archiving read completed\n");
    }
#endif /* UNIV_DEBUG */
...
loop:
...
#ifdef UNIV_DEBUG
  if (log_debug_writes) {
    ib_logger(ib_stream, "Archiving from lsn %lu to lsn %lu\n",
  ...
  log_group_read_log_seg(LOG_ARCHIVE, log_sys->archive_buf,
                         UT_LIST_GET_FIRST(log_sys->log_groups), start_lsn,
                         limit_lsn);
...
function was called. */
static void log_archive_all(ib_recovery_t recovery) /*!< in: recovery flag */
{
  uint64_t present_lsn;
  ulint dummy;
...
static void
log_archive_close_groups(ibool increment_file_count) /*!< in: TRUE if we want to
  ...
  if (trunc_len > 0) {
    ...
    if (log_debug_writes) {
      ib_logger(ib_stream,
                ...
                " in log group %lu\n",
                (ulong)group->archived_file_no + 2, (ulong)group->id);
    }
#endif /* UNIV_DEBUG */
  }
}
...
archiving. */
static void log_archive_margin(void) {
  log_t *log = log_sys;
  ulint age;
  ibool sync;
  ulint dummy;
...
loop:
  ...
  if (shutdown != IB_SHUTDOWN_NO_BUFPOOL_FLUSH &&
      (srv_error_monitor_active || srv_lock_timeout_active ||
       srv_monitor_active)) {
...
ibool log_check_log_recs(const byte *buf, /*!< in: pointer to the start of
                                          the log segment in the
                                          log_sys->buf log buffer */
                         ulint len,       /*!< in: segment length in bytes */
                         uint64_t buf_start_lsn) /*!< in: buffer start lsn */
{
  ...
  auto buf1 = static_cast<byte*>(mem_alloc((end - start) + OS_FILE_LOG_BLOCK_SIZE));
...
ibool log_peek_lsn(
    uint64_t *lsn) /*!< out: if returns TRUE, current lsn is here */
{
...
void log_print(ib_stream_t ib_stream) /*!< in: file where to print */
{
  double time_elapsed;
  time_t current_time;
...
/** Closes a log group. */
static void
log_group_close(log_group_t *group) /*!< in,own: log group to close */
{
  ulint i;
