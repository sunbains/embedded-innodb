...
ib_cb_t recv_pre_rollback_hook = NULL;
...
recv_truncate_group(log_group_t *group,        /*!< in: log group */
                    ib_uint64_t recovered_lsn, /*!< in: recovery succeeded up to
                                               this lsn */
                    ib_uint64_t limit_lsn,     /*!< in: this was the limit for
                                               recovery */
                    ib_uint64_t checkpoint_lsn, /*!< in: recovery was started
                                                from this checkpoint */
                    ib_uint64_t archived_lsn) /*!< in: the log has been archived
                                              up to this lsn */
{
...
void recv_scan_log_seg_for_backup(
    ...
    buf_block_t *block) /*!< in/out: buffer block */
{
  ...
  if ((recv_addr == NULL) || (recv_addr->state == RECV_BEING_PROCESSED) ||
...
void recv_apply_log_recs_for_backup(void) {
  ...
  ib_logger(ib_stream, "InnoDB: Starting an apply batch of log records "
                       "to the database...\n"
                       "InnoDB: Progress in percents: ");
  ...
  for (i = 0; i < n_hash_cells; i++) {
    ...
    while (recv_addr != NULL) {
      ...
      ulint zip_size = fil_space_get_zip_size(recv_addr->space);
      ...
      buf_page_init_for_backup_restore(recv_addr->space, recv_addr->page_no,
                                       zip_size, block);
      ...
      success = fil_extend_space_to_desired_size(&actual_size, recv_addr->space,
                                                 recv_addr->page_no + 1);
      if (!success) {
        srv_panic(DB_ERROR,
      ...
      if (zip_size) {
        error =
            fil_io(OS_FILE_READ, TRUE, recv_addr->space, zip_size,
                   recv_addr->page_no, 0, zip_size, block->page.zip.data, NULL);
        if (error == DB_SUCCESS && !buf_zip_decompress(block, TRUE)) {
          srv_panic(DB_ERROR, "decompressing page");
        }
      } else {
        error =
            fil_io(OS_FILE_READ, TRUE, recv_addr->space, 0, recv_addr->page_no,
                   0, UNIV_PAGE_SIZE, block->frame, NULL);
      }
      ...
      buf_flush_init_for_writing(block->frame, buf_block_get_page_zip(block),
                                 mach_read_ull(block->frame + FIL_PAGE_LSN));

      if (zip_size) {
        error =
            fil_io(OS_FILE_WRITE, TRUE, recv_addr->space, zip_size,
                   recv_addr->page_no, 0, zip_size, block->page.zip.data, NULL);
      } else {
        error =
            fil_io(OS_FILE_WRITE, TRUE, recv_addr->space, 0, recv_addr->page_no,
                   0, UNIV_PAGE_SIZE, block->frame, NULL);
      }
    skip_this_recv_addr:
      recv_addr = HASH_GET_NEXT(addr_hash, recv_addr);
                   ...
                   byte **body)    /*!< out: log record body start */
{
  ...
  *body = new_ptr;
  ...
  new_ptr =
      recv_parse_or_apply_log_rec_body(*type, new_ptr, end_ptr, NULL, NULL);
                             ...
                             to find data in this log block */
{
  ...
  if (start_offset < end_offset) {
    ut_memcpy(recv_sys->buf + recv_sys->len, log_block + start_offset,
...
recv_init_crash_recovery(ib_recovery_t recovery,      /*!< in: recovery flag */
                         ...
                         ib_uint64_t min_flushed_lsn, /*!< in: min flushed lsn
                                                      from data files */
                         ib_uint64_t max_flushed_lsn) /*!< in: max flushed lsn
                                                      from data files */
{
...
ulint recv_recovery_from_checkpoint_start_func(
    ib_recovery_t recovery, /*!< in: recovery flag */
#ifdef UNIV_LOG_ARCHIVE
...
void recv_recovery_rollback_active(void) {
  ...
  if (srv_force_recovery < IB_RECOVERY_NO_TRX_UNDO) {
    ...
    os_thread_create(trx_rollback_or_clean_all_recovered, (void *)&i, NULL);
  }
}
...
void recv_reset_log_files_for_backup(
                         ...
                         divisible by OS_FILE_LOG_BLOCK_SIZE */
{
  ...
  */
  ...
  for (i = 0; i < n_log_files; i++) {
    ...
    ut_snprintf(name, sizeof(name), "%s%s%lu", log_dir, ib_logfile_basename,
                (ulong)i);

    log_file = os_file_create_simple(name, OS_FILE_CREATE, OS_FILE_READ_WRITE,
                                     &success);
    if (!success) {
      srv_panic(DB_ERROR,
    ...
    if (!success) {
      srv_panic(DB_ERROR, "InnoDB: Cannot set %s size to %lu %lu\n", name,
                (ulong)ut_get_high32(log_file_size),
                (ulong)(log_file_size & 0xFFFFFFFFUL));
    }
  ...
  ut_snprintf(name, sizeof(name) - 1, "%s%s%lu", log_dir, ib_logfile_basename,
              (ulong)0);

  log_file =
      os_file_create_simple(name, OS_FILE_OPEN, OS_FILE_READ_WRITE, &success);
  if (!success) {
    srv_panic(DB_ERROR, "InnoDB: Cannot open %s.\n", name);
  }
...
#error "RECV_SCAN_SIZE < LOG_FILE_HDR_SIZE"
  ...
  fil_io(OS_FILE_READ | OS_FILE_LOG, TRUE, group->archive_space_id,
         ...
         0, 0, LOG_FILE_HDR_SIZE, buf, NULL);
                                 ...
                                 server config file */
{
  ...
  if (recv_sys->recovered_lsn < limit_lsn) {
    ...
    err = recv_recovery_from_checkpoint_start(LOG_ARCHIVE, limit_lsn,
                                              IB_UINT64_T_MAX, IB_UINT64_T_MAX);
    if (err != DB_SUCCESS) {
