...
/** Sets the global variable log_fsp_current_free_limit. Also makes a
checkpoint, so that we know that the limit has been written to a log checkpoint
field on disk. */
...
static ib_uint64_t log_buf_pool_get_oldest_modification(void) {
  ib_uint64_t lsn;
...
ib_uint64_t
log_reserve_and_open(ulint len) /*!< in: length of data to be catenated */
{
  log_t *log = log_sys;
...
void log_write_low(byte *str,     /*!< in: string */
...
ib_uint64_t log_close(ib_recovery_t recovery) /*!< in: recovery flag */
{
  byte *log_block;
...
static ulint
log_group_calc_lsn_offset(ib_uint64_t lsn, /*!< in: lsn, must be within 4 GB of
                                           group->lsn */
                          const log_group_t *group) /*!< in: log group */
{
  ib_uint64_t gr_lsn;
  ...
  gr_lsn_size_offset =
      (ib_int64_t)log_group_calc_size_offset(group->lsn_offset, group);
                                  ...
                                  (including the header) */
{
  ...
  if (lsn < first_header_lsn) {
    add_this_many =
        1 + (first_header_lsn - lsn) / (capacity * (ib_int64_t)n_log_files);
    lsn += add_this_many * capacity * (ib_int64_t)n_log_files;
  }
...
void log_group_set_fields(log_group_t *group, /*!< in/out: group */
                          ib_uint64_t lsn)    /*!< in: lsn for which the values
                                              should be    set */
{
...
failure:
  ...
  if (!success) {
    srv_panic(DB_ERROR, "InnoDB: Error: ib_logfiles are too small"
...
log_group_file_header_flush(log_group_t *group, /*!< in: log group */
                            ulint nth_file, /*!< in: header to the nth file in
                                            the log file space */
                            ib_uint64_t start_lsn) /*!< in: log file data starts
                                                   at this lsn */
{
  byte *buf;
  ulint dest_offset;
...
void log_group_write_buf(log_group_t *group, /*!< in: log group */
                         ...
                         ib_uint64_t start_lsn, /*!< in: start lsn of the
...
void log_write_up_to(ib_uint64_t lsn, /*!< in: log sequence number up to which
