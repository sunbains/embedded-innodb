  ...
  while (foreign != NULL) {
    if (ut_strlen(foreign->foreign_table_name) < ut_strlen(table->name)) {
      ...
      foreign->foreign_table_name =
          mem_heap_alloc(foreign->heap, ut_strlen(table->name) + 1);
    }
    ...
    if (strchr(foreign->id, '/')) {
      ...
      if (ut_strlen(foreign->id) >
              ut_strlen(old_name) + ((sizeof dict_ibfk) - 1) &&
          !memcmp(foreign->id, old_name, ut_strlen(old_name)) &&
          !memcmp(foreign->id + ut_strlen(old_name), dict_ibfk,
                  (sizeof dict_ibfk) - 1)) {

        /* This is a generated >= 4.0.18 format id */

        if (strlen(table->name) > strlen(old_name)) {
          foreign->id = mem_heap_alloc(foreign->heap, strlen(table->name) +
                                                          strlen(old_id) + 1);
        }
  ...
  while (foreign != NULL) {
    if (ut_strlen(foreign->referenced_table_name) < ut_strlen(table->name)) {
      ...
      foreign->referenced_table_name =
          mem_heap_alloc(foreign->heap, strlen(table->name) + 1);
    }
...
undo_size_ok:
  ...
  if (!UNIV_UNLIKELY(new_index->type & DICT_UNIVERSAL)) {
    ...
    new_index->stat_n_diff_key_vals = mem_heap_alloc(
        new_index->heap,
        (1 + dict_index_get_n_unique(new_index)) * sizeof(ib_int64_t));
                          ...
                          const char *msg)    /*!< in: the error message */
{
  ...
  dict_print_info_on_foreign_key_in_create_format(ib_stream, NULL, fk, TRUE);
...
ulint dict_foreign_add_to_cache(
...
static ulint dict_create_foreign_constraints_low(
...
col_loop1:
  ...
  index = dict_foreign_find_index(table, column_names, i, NULL, TRUE, FALSE);
...
ulint dict_create_foreign_constraints(
    trx_t *trx,             /*!< in: transaction */
...
ulint dict_foreign_parse_drop_constraints(
...
void dict_update_statistics_low(dict_table_t *table, /*!< in/out: table */
...
void dict_index_name_print(ib_stream_t ib_stream,     /*!< in: output stream */
                           trx_t *trx,                /*!< in: transaction */
