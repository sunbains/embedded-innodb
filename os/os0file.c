...
void os_file_var_init(void) {
  os_aio_segment_wait_events = NULL;
                             ...
                             printed of all errors */
{
  ...
  if (report_all_errors ||
    ...
    if (err == ERROR_PATH_NOT_FOUND) {
      ...
      ib_logger(ib_stream, "InnoDB: The error means that another program"
                           " is using InnoDB's files.\n"
                           "InnoDB: This might be a backup or antivirus"
                           " software or another instance\n"
                           "InnoDB: of this binary."
                           " Please close it to get rid of this error.\n");
    } else if (err == ERROR_WORKING_SET_QUOTA ||
               err == ERROR_NO_SYSTEM_RESOURCES) {
      ib_logger(ib_stream, "InnoDB: The error means that there are no"
                           " sufficient system resources or quota to"
                           " complete the operation.\n");
    } else if (err == ERROR_OPERATION_ABORTED) {
      ib_logger(ib_stream, "InnoDB: The error means that the I/O"
                           " operation has been aborted\n"
                           "InnoDB: because of either a thread exit"
                           " or an application request.\n"
                           "InnoDB: Retry attempt is made.\n");
    } else {
      ib_logger(ib_stream, "InnoDB: Some operating system error numbers"
                           " are described at\n"
                           "InnoDB: "
                           ...
                           and this parameter is TRUE */
{
  ...
  if (err == OS_FILE_DISK_FULL) {
    ...
    if (should_exit) {
      srv_panic(DB_ERROR, "InnoDB: Cannot continue operation.\n");
    }
  }
...
@return	TRUE if we should retry the operation */
static ibool
os_file_handle_error(const char *name,      /*!< in: name of a file or NULL */
                     const char *operation) /*!< in: operation */
{
...
@return	0 on success */
static int os_file_lock(int fd,           /*!< in: file descriptor */
...
/** Creates a temporary file.  This function is like tmpfile(3)/
...
os_file_dir_t
os_file_opendir(const char *dirname,  /*!< in: directory name; it must not
...
int os_file_closedir(os_file_dir_t dir) /*!< in: directory stream */
{
#ifdef __WIN__
  BOOL ret;
...
next_file:
  ...
  if (ret) {
    ...
    info->size = (ib_int64_t)(lpFindFileData->nFileSizeLow) +
                 (((ib_int64_t)(lpFindFileData->nFileSizeHigh)) << 32);

    if (lpFindFileData->dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
  ...
  if (ret) {
    ...
    os_file_handle_error_no_exit(dirname, "readdir_next_file");
    return (-1);
  }
#else
  ulint len;
...
next_file:
  ...
  ut_a(ut_strlen(ent->d_name) < _POSIX_PATH_MAX + 100 - 1);
#else
  ent = readdir(dir);
...
#endif
  ut_a(ut_strlen(ent->d_name) < OS_FILE_MAX_PATH);
  ...
  ut_strcpy(info->name, ent->d_name);

  len = ut_strlen(dirname) + ut_strlen(ent->d_name) + 10;

  full_path = ut_malloc(len);
...
directory gets default permissions. On Unix the permissions are (0770 & ~umask).
If the directory exists already, nothing is done and the call succeeds, unless
the fail_if_exists arguments is true.
@return	TRUE if call succeeds, FALSE on error */

ibool os_file_create_directory(
...
try_again:
  ...
  file = CreateFile((LPCTSTR)name, access, FILE_SHARE_READ | FILE_SHARE_WRITE,
                    /* file can be read and written also
                    by other processes */
                    NULL, /* default security attributes */
                    create_flag, attributes, NULL); /*!< no template file */
    ...
    ibool *success)    /*!< out: TRUE if succeed, FALSE if error */
{
  ...
  file = CreateFile((LPCTSTR)name, access, share_mode,
                    NULL, /* default security attributes */
                    create_flag, attributes, NULL); /*!< no template file */
...
os_file_t
os_file_create(const char *name,  /*!< in: name of the file or path as a
                                  null-terminated string */
               ulint create_mode, /*!< in: OS_FILE_OPEN if an existing file
                              is opened (if does not exist, error), or
                              OS_FILE_CREATE if a new file is created
...
try_again:
  ...
  file = CreateFile((LPCTSTR)name, GENERIC_READ | GENERIC_WRITE, /* read and
                                                                 write access */
                    share_mode, /* File can be read also by other
                                ...
                                start 2 instances of the binary on the
  ...
  *success = TRUE;
  ...
  if (create_mode != OS_FILE_OPEN_RAW && os_file_lock(file, name)) {
    ...
    if (create_mode == OS_FILE_OPEN_RETRY) {
      int i;
      ut_print_timestamp(ib_stream);
      ib_logger(ib_stream, "  InnoDB: Retrying to lock"
                           " the first data file\n");
      for (i = 0; i < 100; i++) {
      ...
      ut_print_timestamp(ib_stream);
      ib_logger(ib_stream, "  InnoDB: Unable to open the first "
                           "data file\n");
    }
...
ibool os_file_delete_if_exists(
...
ibool os_file_rename(const char *oldpath, /*!< in: old file path as a
                                          null-terminated string */
...
ibool os_file_close(os_file_t file) /*!< in, own: handle to a file */
{
#ifdef __WIN__
  BOOL ret;
...
ibool os_file_close_no_error_handling(
    ...
    ulint size_high)  /*!< in: most significant 32 bits of size */
{
  ...
  while (current_size < desired_size) {
    ...
    if ((ib_int64_t)(current_size + n_bytes) /
            (ib_int64_t)(100 * 1024 * 1024) !=
        current_size / (ib_int64_t)(100 * 1024 * 1024)) {

      ib_logger(
          ib_stream, " %lu00",
          (ulong)((current_size + n_bytes) / (ib_int64_t)(100 * 1024 * 1024)));
    }
...
ibool os_file_set_eof(FILE *file) /*!< in: file to be truncated */
{
#ifdef __WIN__
...
static int os_file_fsync(os_file_t file) /*!< in: handle to a file */
{
  int ret;
  int failures;
  ibool retry;
  ...
  do {
    ...
    if (ret == -1 && errno == ENOLCK) {
      ...
      if (failures % 100 == 0) {
        ...
        ut_print_timestamp(ib_stream);
        ib_logger(ib_stream, "  InnoDB: fsync(): "
...
ibool os_file_flush(os_file_t file) /*!< in, own: handle to a file */
{
#ifdef __WIN__
  BOOL ret;
...
ibool os_file_read(os_file_t file,    /*!< in: handle to a file */
                   void *buf,         /*!< in: buffer where to read */
                   ulint offset,      /*!< in: least significant 32 bits of file
...
ibool os_file_read_no_error_handling(
    os_file_t file,    /*!< in: handle to a file */
    void *buf,         /*!< in: buffer where to read */
    ulint offset,      /*!< in: least significant 32 bits of file
                       offset where to read */
...
void os_file_read_string(FILE *file, /*!< in: file to read from */
...
ibool os_file_write(const char *name, /*!< in: name of the file or path as a
                                      null-terminated string */
                    os_file_t file,   /*!< in: handle to a file */
...
retry:
  ...
  if (ret2 == 0xFFFFFFFF && GetLastError() != NO_ERROR) {
    ...
    ib_logger(ib_stream,
              ...
              "InnoDB: "
              "Check InnoDB website for details\n",
              name, (ulong)offset_high, (ulong)offset, (ulong)GetLastError());
  ...
  if (!os_has_said_disk_full) {
    ...
    ib_logger(ib_stream,
              ...
              name, (ulong)offset_high, (ulong)offset, (ulong)n, (ulong)len,
              (ulong)err);
...
ibool os_file_get_status(const char *path, /*!< in:	pathname of the file */
                         os_file_stat_t *stat_info) /*!< information of a file
                                                    in a directory */
{
#ifdef __WIN__
  int ret;
...
ibool os_file_create_subdirs_if_needed(const char *path) /*!< in: path name */
{
  char *subdir;
...
void os_aio_init(ulint n_per_seg,    /*<! in: maximum number of pending aio
...
shutdown. */
static void os_aio_array_wake_win_aio_at_shutdown(
...
ibool os_aio(ulint type,           /*!< in: OS_FILE_READ or OS_FILE_WRITE */
  ...
  if (os_aio_use_native_aio) {
    if ((ret && len == n) || (!ret && GetLastError() == ERROR_IO_PENDING)) {
      ...
      if (mode == OS_AIO_SYNC) {
        ...
        retval = os_aio_windows_handle(ULINT_UNDEFINED, slot->pos, &dummy_mess1,
...
ibool os_aio_windows_handle(
    ...
    void **message2, ulint *type) /*!< out: OS_FILE_WRITE or ..._READ */
{
  ...
  if (array == os_aio_sync_array) {
    ...
    os_set_io_thread_op_info(orig_seg, "wait Windows aio");
    i = os_event_wait_multiple(n, (array->native_events) + segment * n);
  }
  ...
  if (orig_seg != ULINT_UNDEFINED) {
    os_set_io_thread_op_info(orig_seg, "get windows aio return value");
  }
  ...
  *type = slot->type;
  ...
  if (retry) {
    ...
    switch (slot->type) {
    case OS_FILE_WRITE:
      ret = WriteFile(slot->file, slot->buf, slot->len, &len, &(slot->control));

      break;
    case OS_FILE_READ:
      ret = ReadFile(slot->file, slot->buf, slot->len, &len, &(slot->control));
...
loop:
  ...
  for (i = 0; i < array->n_slots; i++) {
    ...
    if (slot->reserved) {
      n_reserved++;
#if 0
			ib_logger(ib_stream, "Reserved slot, messages %p %p\n",
				(void*) slot->message1,
				(void*) slot->message2);
#endif
      ut_a(slot->len > 0);
