                               ...
                               thread, or NULL */
{
  ...
  thread = CreateThread(NULL, /* no security attributes */
                        ...
                        (LPTHREAD_START_ROUTINE)start_f, arg,
  ...
  ret =
      pthread_attr_setstacksize(&attr, (size_t)(PTHREAD_STACK_MIN + 32 * 1024));
  if (ret) {
    srv_panic(ret,
...
#endif
#ifdef __NETWARE__
  ret = pthread_attr_setstacksize(&attr, (size_t)NW_THD_STACKSIZE);
  if (ret) {
    srv_panic(ret,
              "InnoDB: Error: pthread_attr_setstacksize"
              " returned %d\n",
...
#endif
  if (ret) {
    srv_panic(ret, "InnoDB: Error: pthread_create returned %d\n", ret);
  }
...
void os_thread_exit(void *exit_value) /*!< in: exit value; in Windows this void*
                                      is cast as a DWORD */
{
#ifndef __WIN__
  int ret;
