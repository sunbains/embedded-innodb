                      ...
                      ulint i) /*!< in: index of buf_pool->zip_free[] */
{
  ...
  if (b)
    UNIV_MEM_VALID(b, BUF_BUDDY_LOW << i);
...
#ifdef UNIV_DEBUG_VALGRIND
  if (b)
    UNIV_MEM_FREE(b, BUF_BUDDY_LOW << i);
                           ...
                           ulint i) /*!< in: index of buf_pool->zip_free[] */
{
  ...
  if (prev)
    UNIV_MEM_VALID(prev, BUF_BUDDY_LOW << i);
  if (next)
    UNIV_MEM_VALID(next, BUF_BUDDY_LOW << i);
...
#ifdef UNIV_DEBUG_VALGRIND
  if (prev)
    UNIV_MEM_FREE(prev, BUF_BUDDY_LOW << i);
  if (next)
    UNIV_MEM_FREE(next, BUF_BUDDY_LOW << i);
#endif /* UNIV_DEBUG_VALGRIND */
}
...
static void *
buf_buddy_alloc_zip(ulint i) /*!< in: index of buf_pool->zip_free[] */
{
  buf_page_t *bpage;
  ...
  ut_d(UT_LIST_VALIDATE(
      list, buf_page_t, buf_pool->zip_free[i],
      ut_ad(buf_page_get_state(ut_list_node_313) == BUF_BLOCK_ZIP_FREE)));
#endif /* !UNIV_DEBUG_VALGRIND */
  ...
  if (bpage) {
    ...
    if (bpage) {
      buf_page_t *buddy =
          (buf_page_t *)(((char *)bpage) + (BUF_BUDDY_LOW << i));
...
static void
buf_buddy_block_free(void *buf) /*!< in: buffer frame to deallocate */
{
...
static void buf_buddy_block_register(
...
static void *
buf_buddy_alloc_from(void *buf, /*!< in: a block that is free to use */
                     ulint i,   /*!< in: index of buf_pool->zip_free[] */
                   ...
                   ulint i)   /*!< in: index of buf_pool->zip_free[] */
{
  ...
  if (size >= PAGE_ZIP_MIN_SIZE) {
    ...
    bpage = buf_page_hash_get(
        ...
        mach_read_from_4((const byte *)src + FIL_PAGE_OFFSET));
    ...
    success:
      ...
      {
        buf_buddy_stat_t *buddy_stat = &buf_buddy_stat[i];
...
void buf_buddy_free_low(void *buf, /*!< in: block to be freed, must not be
...
buddy_nonfree:
  ...
  if (bpage) {
    ...
    buddy = (buf_page_t *)buf_buddy_get(((byte *)bpage), BUF_BUDDY_LOW << i);
  ...
  if (i < buf_buddy_get_slot(PAGE_ZIP_MIN_SIZE)) {
    ...
    for (; b < b_end; b++) {
      ...
      if ((b->state == BUF_BLOCK_ZIP_PAGE || b->state == BUF_BLOCK_ZIP_DIRTY) &&
          b->space > 0 && b->space < 1000) {
        ib_logger(ib_stream, "buddy dirty %p %u (%u,%u) %p,%lu\n", (void *)b,
                  b->state, b->space, b->offset, buf, i);
      }
    }
#endif /* !UNIV_DEBUG_VALGRIND */
    ...
    bpage->list pointers. */
    for (c = (char *)buf + (BUF_BUDDY_LOW << i); c-- > (char *)buf;) {
      *c = ~*c ^ i;
    }
  } else {
