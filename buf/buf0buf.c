  ...
  if (recv_lsn_checks_on) {
    ...
    if (log_peek_lsn(&current_lsn) &&
      ...
      ib_logger(ib_stream,
                ...
                mach_read_ull(read_buf + FIL_PAGE_LSN), current_lsn);
    }
  }
#endif
  ...
  if (io_type == BUF_IO_READ) {
    ...
    corrupt:
      ...
      if (srv_force_recovery < IB_RECOVERY_IGNORE_CORRUPT) {
        srv_panic(DB_CORRUPTION, "InnoDB: Ending processing because of"
...
void buf_print_io(ib_stream_t ib_stream) /*!< in/out: buffer where to print */
{
  time_t current_time;
  double time_elapsed;
  ulint n_gets_diff;
  ...
  ib_logger(ib_stream,
            ...
            (ulong)UT_LIST_GET_LEN(buf_pool->LRU), (ulong)buf_pool->LRU_old_len,
            (ulong)UT_LIST_GET_LEN(buf_pool->flush_list),
  ...
  if (n_gets_diff) {
    ...
    ib_logger(ib_stream, "No buffer pool page gets since the last printout\n");
  }
  ...
  ib_logger(ib_stream,
            ...
            UT_LIST_GET_LEN(buf_pool->unzip_LRU), buf_LRU_stat_sum.io,
            buf_LRU_stat_cur.io, buf_LRU_stat_sum.unzip,
