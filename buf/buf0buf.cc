...
buf_block_t *Buf_pool::get(Request &req, buf_block_t *guess) {
  ...
  for (;;) {
    ...
    if (block != nullptr) {
      if (page_id.m_page_no != block->m_page.m_page_no || page_id.m_space_id != block->m_page.m_space || block->get_state() != BUF_BLOCK_FILE_PAGE) {
