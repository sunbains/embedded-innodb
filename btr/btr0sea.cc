...
static void btr_search_build_page_hash_index(
    ...
    ulint n_bytes,       /*!< in: hash this many bytes from the next
                         field */
    bool left_side);     /*!< in: hash for searches from left side? */
...
btr_search_t *
btr_search_info_create(mem_heap_t *heap) /*!< in: heap where created */
{
  btr_search_t *info;
...
ulint btr_search_info_get_ref_count(btr_search_t *info) /*!< in: search info. */
{
  ulint ret;
...
are consistent. */
static void btr_search_info_update_hash(
...
static bool
btr_search_update_block_hash_info(btr_search_t *info, /*!< in: search info */
...
index. */
static void btr_search_update_hash_ref(
    ...
    btr_cur_t *cursor)  /*!< in: cursor */
{
  ...
  if ((info->n_hash_potential > 0) &&
    ...
    fold = rec_fold(
        rec,
        rec_get_offsets(rec, cursor->index, offsets_, ULINT_UNDEFINED, &heap),
    ...
    ha_insert_for_fold(btr_search_sys->hash_index, fold, block, rec);
  }
}
    ...
    btr_cur_t *cursor)  /*!< in: cursor which was just positioned */
{
  ...
  if (build_index) {
    ...
    btr_search_build_page_hash_index(cursor->index, block, params2[0],
                                     params2[1], params2[2]);
    mem_free(params);
  }
}
...
static bool
btr_search_check_guess(btr_cur_t *cursor, /*!< in: guessed cursor position */
                       ...
                       mtr_t *mtr) /*!< in: mtr */
{
  ...
  cmp = page_cmp_dtuple_rec_with_match(cursor->index->cmp_ctx, tuple, rec,
                                       offsets, &match, &bytes);
  ...
  if ((mode == PAGE_CUR_G) || (mode == PAGE_CUR_GE)) {
    ...
    cmp = page_cmp_dtuple_rec_with_match(cursor->index->cmp_ctx, tuple,
                                         prev_rec, offsets, &match, &bytes);
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  if (unlikely(info->n_hash_potential == 0)) {
  ...
  if (unlikely(dtuple_get_n_fields(tuple) <
  ...
  if (likely(!has_search_latch)) {
    rw_lock_s_lock(&btr_search_latch);

    if (unlikely(!btr_search_enabled)) {
      goto failure_unlock;
    }
  }
  ...
  if (likely(!has_search_latch)) {
    ...
    if (unlikely(!buf_page_get_known_nowait(latch_mode, block, BUF_MAKE_YOUNG,
  ...
  right. */
  if (expect(ut_dulint_cmp(index_id, btr_page_get_index_id(block->frame)), 0) ||
  ...
  if (likely(info->n_hash_potential < BTR_SEARCH_BUILD_LIMIT + 5)) {
  ...
  btr_search_n_succ++;
#endif
  if (likely(!has_search_latch) && buf_page_peek_if_too_old(&block->page)) {
...
retry:
  ...
  while (!page_rec_is_supremum(rec)) {
    offsets =
        rec_get_offsets(rec, index, offsets, n_fields + (n_bytes > 0), &heap);
  ...
  if (unlikely(block->curr_n_fields != n_fields) ||
      unlikely(block->curr_n_bytes != n_bytes)) {
    ...
    bool left_side)      /*!< in: hash for searches from left side? */
{
  ...
  if (dict_index_get_n_unique_in_tree(index) < n_fields ||
      (dict_index_get_n_unique_in_tree(index) == n_fields && n_bytes > 0)) {
    return;
  }
  ...
  for (;;) {
    ...
    next_fold = rec_fold(next_rec, offsets, n_fields, n_bytes, index_id);
...
void btr_search_move_or_delete_hash_entries(
                       ...
                       to the cursor */
{
  ...
  if ((cursor->flag == BTR_CUR_HASH) &&
    ...
    ha_search_and_update_if_found(table, cursor->fold, rec, block,
                       ...
                       to the cursor */
{
  ...
  offsets =
      rec_get_offsets(ins_rec, cursor->index, offsets, ULINT_UNDEFINED, &heap);
  ...
  for (i = 0; i < cell_count; i++) {
    ...
    for (; node != NULL; node = node->next) {
      ...
      offsets = rec_get_offsets(
          ...
          block->curr_n_fields + (block->curr_n_bytes > 0), &heap);

      if (!block->is_hashed ||
          node->fold != rec_fold((rec_t *)(node->data), offsets,
        ...
        ib_logger(ib_stream,
                  ...
                  (ulong)block->curr_n_fields, (ulong)block->curr_n_bytes,
