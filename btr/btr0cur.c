    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  for (;;) {
    buf_block_t *block;
    page_t *page;
    block = buf_page_get_gen(space, zip_size, page_no, RW_NO_LATCH, NULL,
    ...
    if (UNIV_LIKELY(entry->n_fields >= n_uniq) &&
        UNIV_UNLIKELY(REC_NODE_PTR_SIZE +
                          rec_get_converted_size_comp_prefix(
                              dict_index, entry->fields, n_uniq, NULL)
    ...
    roll_ptr_t *roll_ptr) /*!< out: roll pointer */
{
  ...
  err =
      trx_undo_report_row_operation(flags, TRX_UNDO_MODIFY_OP, thr, dict_index,
                                    NULL, update, cmpl_info, rec, roll_ptr);
  return (err);
}
    ...
    dict_index_t *dict_index) /*!< in: index corresponding to page */
{
  ...
  offsets = rec_get_offsets(rec, dict_index, NULL, ULINT_UNDEFINED, &heap);
                         ...
                         latching any further pages */
{
  ...
  if (block->is_hashed) {
    ...
    if (!dict_index_is_clust(dict_index) ||
        row_upd_changes_ord_field_binary(NULL, dict_index, update)) {
                         ...
                         latching any further pages */
{
  ...
  *big_rec = NULL;
  ...
  err = btr_cur_pessimistic_insert(
      ...
      new_entry, &rec, &dummy_big_rec, n_ext, NULL, mtr);
  ut_a(rec);
  ut_a(err == DB_SUCCESS);
...
ulint btr_cur_del_mark_set_sec_rec(ulint flags,       /*!< in: locking flag */
                                   btr_cur_t *cursor, /*!< in: cursor */
                                   ibool val,         /*!< in: value to set */
                                   que_thr_t *thr,    /*!< in: query thread */
                                   mtr_t *mtr)        /*!< in: mtr */
...
ib_int64_t btr_estimate_n_rows_in_range(
    ...
    dict_index_t *index) /*!< in: index */
{
  ...
  for (j = 0; j <= n_cols; j++) {
    index->stat_n_diff_key_vals[j] =
        ((n_diff[j] * (ib_int64_t)index->stat_n_leaf_pages + n_sample_pages -
                          ...
                          mtr_t *mtr) /*!< in: mini-transaction to commit */
{
  ...
  if (buf_block_get_state(block) == BUF_BLOCK_FILE_PAGE &&
    ...
    if (buf_LRU_free_block(&block->page, all, NULL) != BUF_LRU_FREED && all &&
        block->page.zip.data) {
