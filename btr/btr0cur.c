	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	if (rw_lock_get_writer(&btr_search_latch) == RW_LOCK_NOT_LOCKED
	    ...
	    && btr_search_guess_on_hash(dict_index, info, tuple, mode,
					latch_mode, cursor,
...
retry_page_get:
		...
		if (block == NULL) {
			...
			if (ibuf_insert(tuple, dict_index, space, zip_size,
					page_no, cursor->thr)) {
		...
		ut_ad(0 == ut_dulint_cmp(dict_index->id,
		...
		page_cur_search_with_match(block, dict_index, tuple, page_mode,
					   &up_match, &up_bytes,
					   &low_match, &low_bytes,
					   page_cursor);
				...
				successor record */
{
	...
	if (dict_index_is_clust(dict_index) && !dict_index_is_ibuf(dict_index)) {
		...
		if (!(flags & BTR_KEEP_SYS_FLAG)) {
			...
			row_upd_index_entry_sys_field(entry, dict_index,
		...
		if (UNIV_LIKELY(entry->n_fields >= n_uniq)
		    && UNIV_UNLIKELY(REC_NODE_PTR_SIZE
				     + rec_get_converted_size_comp_prefix(
					     dict_index, entry->fields, n_uniq,
					     NULL)
...
fail_err:
		...
		if (big_rec_vec) {
			dtuple_convert_back_big_rec(dict_index, entry,
						    big_rec_vec);
		}
...

	{
		...
		*rec = page_cur_tuple_insert(page_cursor, entry, dict_index,
					     n_ext, mtr);
	...
	if (UNIV_UNLIKELY(!*rec) && UNIV_LIKELY(!reorg)) {
		...
		page_cur_search(block, dict_index, entry,
				PAGE_CUR_LE, page_cursor);
	...
	*big_rec = NULL;
	...
	if (page_rec_needs_ext(rec_get_converted_size(dict_index, entry, n_ext),
		...
		if (big_rec_vec == NULL) {
			...
			if (n_extents > 0) {
				fil_space_release_free_extents(dict_index->space,
							       n_reserved);
			}
			return(DB_TOO_BIG_RECORD);
		}
	...
	roll_ptr_t*	roll_ptr)/*!< out: roll pointer */
{
	...
	if (!(flags & BTR_NO_LOCKING_FLAG)) {
		...
		err = lock_clust_rec_modify_check_and_lock(
			...
			rec_get_offsets(rec, dict_index, offsets_,
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	log_ptr = mlog_open_and_write_index(mtr, rec, dict_index,
					    page_is_comp(page)
	...
	log_ptr = row_upd_write_sys_vals_to_log(dict_index, trx, roll_ptr,
						log_ptr, mtr);
	...
	mtr_t*		mtr)	/*!< in: mini-transaction */
{
	...
	if (page_zip_available(page_zip, dict_index_is_clust(dict_index),
			       length, create)) {
		return(TRUE);
	}
				...
				latching any further pages */
{
	...
	if (block->is_hashed) {
		...
		if (!dict_index_is_clust(dict_index)
		    || row_upd_changes_ord_field_binary(NULL, dict_index,
							update)) {
	...
	if (!(flags & BTR_KEEP_SYS_FLAG)) {
		row_upd_rec_sys_fields(rec, NULL,
				       dict_index, offsets, trx, roll_ptr);
	}
	...
	btr_cur_update_in_place_log(flags, rec, dict_index, update,
				    trx, roll_ptr, mtr);
