...
static buf_block_t *
btr_root_block_get(dict_index_t *dict_index, /*!< in: dict_index tree */
                   mtr_t *mtr)               /*!< in: mtr */
{
  ulint space;
  ulint zip_size;
  ...
  ut_a((bool)!!page_is_comp(buf_block_get_frame(block)) ==
...
page_t *btr_root_get(dict_index_t *dict_index, /*!< in: dict_index tree */
                     mtr_t *mtr)               /*!< in: mtr */
{
...
rec_t *
btr_get_prev_user_rec(rec_t *rec, /*!< in: record on leaf level */
                      mtr_t *mtr) /*!< in: mtr holding a latch on the page, and
                                  if needed, also to the previous page */
{
  page_t *page;
  page_t *prev_page;
  ulint prev_page_no;
  ...
  if (prev_page_no != FIL_NULL) {
    ...
    prev_block = buf_page_get_with_no_latch(space, zip_size, prev_page_no, mtr);
...
rec_t *
btr_get_next_user_rec(rec_t *rec, /*!< in: record on leaf level */
                      mtr_t *mtr) /*!< in: mtr holding a latch on the page, and
                                  if needed, also to the next page */
{
  page_t *page;
  page_t *next_page;
  ulint next_page_no;
  ...
  if (next_page_no != FIL_NULL) {
    ...
    next_block = buf_page_get_with_no_latch(space, zip_size, next_page_no, mtr);
...
static buf_block_t *
btr_page_alloc_for_ibuf(dict_index_t *dict_index, /*!< in: dict_index tree */
                        mtr_t *mtr)               /*!< in: mtr */
{
  ...
  new_block = buf_page_get(dict_index_get_space(dict_index),
                           dict_table_zip_size(dict_index->table),
...
buf_block_t *btr_page_alloc(dict_index_t *dict_index, /*!< in: dict_index */
                            ulint hint_page_no,  /*!< in: hint of a good page */
                            byte file_direction, /*!< in: direction where a
                                                 possible page split is made */
                            ...
                            mtr_t *mtr)  /*!< in: mtr */
{
  ...
  new_block = buf_page_get(dict_index_get_space(dict_index),
                           dict_table_zip_size(dict_index->table), new_page_no,
                           RW_X_LATCH, mtr);
  buf_block_dbg_add_level(new_block, SYNC_TREE_NODE_NEW);
...
ulint btr_get_size(dict_index_t *dict_index, /*!< in: dict_index */
...
static void btr_page_free_for_ibuf(
    dict_index_t *dict_index, /*!< in: dict_index tree */
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  fseg_free_page(seg_header, buf_block_get_space(block),
...
void btr_page_free(dict_index_t *dict_index, /*!< in: dict_index tree */
                   buf_block_t *block, /*!< in: block to be freed, x-latched */
                   mtr_t *mtr)         /*!< in: mtr */
{
  ulint level;
...
inline void btr_node_ptr_set_child_page_no(
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  return (btr_block_get(space, dict_table_zip_size(dict_index->table), page_no,
                        RW_X_LATCH, mtr));
}
    ...
    mtr_t *mtr)        /*!< in: mtr */
{
  ...
  ut_ad(
      mtr_memo_contains(mtr, dict_index_get_lock(dict_index), MTR_MEMO_X_LOCK));
  ...
  btr_cur_search_to_nth_level(dict_index, level + 1, tuple, PAGE_CUR_LE,
                              BTR_CONT_MODIFY_TREE, cursor, 0, file, line, mtr);

  node_ptr = btr_cur_get_rec(cursor);
  ut_ad(!page_rec_is_comp(node_ptr) ||
        rec_get_status(node_ptr) == REC_STATUS_NODE_PTR);
  offsets =
      rec_get_offsets(node_ptr, dict_index, offsets, ULINT_UNDEFINED, &heap);
  ...
  if (unlikely(btr_node_ptr_get_child_page_no(node_ptr, offsets) != page_no)) {
    ...
    offsets =
        rec_get_offsets(print_rec, dict_index, offsets, ULINT_UNDEFINED, &heap);
    page_rec_print(print_rec, offsets);
    offsets =
        rec_get_offsets(node_ptr, dict_index, offsets, ULINT_UNDEFINED, &heap);
    page_rec_print(node_ptr, offsets);

    ib_logger(ib_stream, "InnoDB: You should dump + drop + reimport the table"
                         " to fix the\n"
...
static void
btr_page_get_father(dict_index_t *dict_index, /*!< in: b-tree index */
                    buf_block_t *block, /*!< in: child page in the index */
                    mtr_t *mtr,         /*!< in: mtr */
                    btr_cur_t *cursor)  /*!< out: cursor on node pointer record,
                                        its page x-latched */
                 ...
                 mtr_t *mtr)               /*!< in: mini-transaction handle */
{
  ...
  if (type & DICT_IBUF) {
    ...
    page_no = fseg_alloc_free_page(buf_block_get_frame(ibuf_hdr_block) +
  ...
  if (likely_null(page_zip)) {
    page = page_create_zip(block, dict_index, 0, mtr);
  } else {
...
top_loop:
  ...
  finished =
      fseg_free_step_not_header(root + PAGE_HEADER + PAGE_BTR_SEG_TOP, &mtr);
  mtr_commit(&mtr);
...
static bool
btr_page_reorganize_low(bool recovery,      /*!< in: true if called in recovery:
                        ...
                        mtr_t *mtr)               /*!< in: mtr */
{
  ...
  if (likely_null(page_zip) &&
      unlikely(!page_zip_compress(page_zip, page, dict_index, NULL))) {
  ...
  if (unlikely(data_size1 != data_size2) ||
      unlikely(max_ins_size1 != max_ins_size2)) {
    ...
    ib_logger(ib_stream,
              ...
              (unsigned long)max_ins_size1, (unsigned long)max_ins_size2);
  } else {
    success = true;
  }
...
bool btr_page_reorganize(buf_block_t *block, /*!< in: page to be reorganized */
                         dict_index_t *dict_index, /*!< in: record descriptor */
                         mtr_t *mtr)               /*!< in: mtr */
{
...
btr_page_empty(buf_block_t *block,       /*!< in: page to be emptied */
               ...
               dict_index_t *dict_index, /*!< in: index of the page */
               ulint level, /*!< in: the B-tree level of the page */
               mtr_t *mtr)  /*!< in: mtr */
{
  page_t *page = buf_block_get_frame(block);
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  btr_page_create(new_block, new_page_zip, dict_index, level, mtr);
  ...
  if (0
      ...
      || new_page_zip
#endif /* UNIV_ZIP_COPY */
      || unlikely(!page_copy_rec_list_end(new_block, root_block,
                                          page_get_infimum_rec(root),
                                          dict_index, mtr))) {
    ut_a(new_page_zip);

    /* Copy the page byte for byte. */
    page_zip_copy_recs(new_page_zip, new_page, root_page_zip, root, dict_index,
                       mtr);
    ...
    btr_search_move_or_delete_hash_entries(new_block, root_block, dict_index);
  }
  ...
  node_ptr =
      dict_index_build_node_ptr(dict_index, rec, new_page_no, heap, level);
  ...
  btr_page_empty(root_block, root_page_zip, dict_index, level + 1, mtr);
  ...
  node_ptr_rec =
      page_cur_tuple_insert(page_cursor, node_ptr, dict_index, 0, mtr);
    ...
    ulint n_ext)           /*!< in: number of externally stored columns */
{
  ...
  if (likely_null(page_zip)) {
    ...
    if (likely(free_space > (ulint)free_space_zip)) {
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  btr_cur_search_to_nth_level(dict_index, level, tuple, PAGE_CUR_LE,
                              BTR_CONT_MODIFY_TREE, &cursor, 0, file, line,
                              mtr);

  err = btr_cur_pessimistic_insert(
      BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_NO_UNDO_LOG_FLAG, &cursor,
...
index tree. */
static void
btr_attach_half_pages(dict_index_t *dict_index, /*!< in: the index tree */
                      ...
                      mtr_t *mtr)             /*!< in: mtr */
{
  ...
  if (direction == FSP_DOWN) {
    ...
    offsets =
        btr_page_get_father_block(NULL, heap, dict_index, block, mtr, &cursor);
  ...
  node_ptr_upper = dict_index_build_node_ptr(dict_index, split_rec,
  ...
  if (prev_page_no != FIL_NULL) {
    buf_block_t *prev_block =
        btr_block_get(space, zip_size, prev_page_no, RW_X_LATCH, mtr);
#ifdef UNIV_BTR_DEBUG
  ...
  if (next_page_no != FIL_NULL) {
    buf_block_t *next_block =
        btr_block_get(space, zip_size, next_page_no, RW_X_LATCH, mtr);
#ifdef UNIV_BTR_DEBUG
...
static bool
btr_page_tuple_smaller(btr_cur_t *cursor,     /*!< in: b-tree cursor */
...
func_start:
  ...
  if (split_rec) {
    ...
    if (unlikely(!insert_left && new_page_zip && n_iterations > 0)) {
  ...
  if (split_rec) {
    ...
    mem_free(buf);
    insert_will_fit = !new_page_zip && btr_page_insert_fits(cursor, NULL, NULL,
                                                            tuple, n_ext, heap);
  }
  ...
  if (direction == FSP_DOWN
    ...
    if (0
        ...
        || page_zip
#endif /* UNIV_ZIP_COPY */
        || unlikely(!page_move_rec_list_start(new_block, block, move_limit,
                                              cursor->index, mtr))) {
    ...
    if (0
        ...
        || page_zip
#endif /* UNIV_ZIP_COPY */
        || unlikely(!page_move_rec_list_end(new_block, block, move_limit,
                                            cursor->index, mtr))) {
      /* For some reason, compressing new_page failed,
      even though it should contain fewer records than
      the original page.  Copy the page byte for byte
...
func_exit:
  ...
  if (!dict_index_is_clust(cursor->index) && page_is_leaf(page)) {
    ibuf_update_free_bits_for_two_pages_low(buf_block_get_zip_size(left_block),
...
static void
btr_level_list_remove(ulint space,    /*!< in: space where removed */
                      ulint zip_size, /*!< in: compressed page size in bytes
                                     or 0 for uncompressed pages */
...
minimum record. */
inline void btr_set_min_rec_mark_log(
    rec_t *rec, /*!< in: record */
...
byte *
btr_parse_set_min_rec_mark(byte *ptr,     /*!< in: buffer */
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  compressed = btr_cur_pessimistic_delete(&err, true, &cursor, RB_NONE, mtr);
  ut_a(err == DB_SUCCESS);
...
static void
btr_lift_page_up(dict_index_t *dict_index, /*!< in: index tree */
                 ...
                 mtr_t *mtr)         /*!< in: mtr */
{
...

  {
    ...
    for (b = father_block; buf_block_get_page_no(b) != root_page_no;) {
      ...
      offsets =
          btr_page_get_father_block(offsets, heap, dict_index, b, mtr, &cursor);
  ...
  btr_page_empty(father_block, father_page_zip, dict_index, page_level, mtr);
  ...
  if (0
...
#endif /* UNIV_ZIP_COPY */
      ||
      unlikely(!page_copy_rec_list_end(
          father_block, block, page_get_infimum_rec(page), dict_index, mtr))) {
    const page_zip_des_t *page_zip = buf_block_get_page_zip(block);
    ut_a(father_page_zip);
    ut_a(page_zip);

    /* Copy the page byte for byte. */
    page_zip_copy_recs(father_page_zip, father_page, page_zip, page, dict_index,
                       mtr);
    ...
    btr_search_move_or_delete_hash_entries(father_block, block, dict_index);
  }
...
bool btr_compress(btr_cur_t *cursor, /*!< in: cursor on the page to merge or
                  ...
                  mtr_t *mtr)        /*!< in: mtr */
{
  ...
  heap = mem_heap_create(100);
  offsets = btr_page_get_father_block(NULL, heap, dict_index, block, mtr,
                                      &father_cursor);
  ...
  if (is_left) {
    ...
    merge_block = btr_block_get(space, zip_size, left_page_no, RW_X_LATCH, mtr);
  ...
  max_ins_size_reorg =
      page_get_max_insert_size_after_reorganize(merge_page, n_recs);
  ...
  err_exit:
    ...
    if (zip_size && page_is_leaf(merge_page) &&
        !dict_index_is_clust(dict_index)) {
  ...
  if (is_left) {
    rec_t *orig_pred = page_copy_rec_list_start(
        merge_block, block, page_get_supremum_rec(page), dict_index, mtr);
    ...
    if (likely_null(merge_page_zip)) {
      ...
      static_assert(FIL_NULL == 0xffffffff, "error FIL_NULL != 0xffffffff");
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  btr_page_empty(block, buf_block_get_page_zip(block), dict_index, 0, mtr);
  ...
  if (!dict_index_is_clust(dict_index)) {
    ...
    if (page_is_leaf(buf_block_get_frame(block))) {
      ...
      page_set_max_trx_id(block, buf_block_get_page_zip(block), max_trx_id,
                          mtr);
    }
  }
}
...
bool btr_check_node_ptr(dict_index_t *index, /*!< in: index tree */
                        buf_block_t *block,  /*!< in: index page */
                        mtr_t *mtr)          /*!< in: mtr */
{
  mem_heap_t *heap;
  ...
  ut_a(!cmp_dtuple_rec(index->cmp_ctx, tuple, btr_cur_get_rec(&cursor),
                       offsets));
func_exit:
  mem_heap_free(heap);
...
static void
btr_index_rec_validate_report(const page_t *page, /*!< in: index page */
                                                 ...
                                                 record and page on error */
{
  ...
  if (unlikely((bool)!!page_is_comp(page) !=
    ...
    ib_logger(ib_stream, "InnoDB: compact flag=%lu, should be %lu\n",
              ...
              (ulong)dict_table_is_comp(dict_index->table));
  ...
  for (i = 0; i < n; i++) {
    ulint fixed_size = dict_col_get_fixed_size(
        dict_index_get_nth_col(dict_index, i), page_is_comp(page));
    ...
    if ((dict_index_get_nth_field(dict_index, i)->prefix_len == 0 &&
         len != UNIV_SQL_NULL && fixed_size && len != fixed_size) ||
        (dict_index_get_nth_field(dict_index, i)->prefix_len > 0 &&
         len != UNIV_SQL_NULL &&
...
static void
btr_validate_report1(dict_index_t *dict_index, /*!< in: index */
...
static void
btr_validate_report2(const dict_index_t *dict_index, /*!< in: index */
                     ulint level,                    /*!< in: B-tree level */
                     const buf_block_t *block1, /*!< in: first index page */
                     const buf_block_t *block2) /*!< in: second index page */
{
  ib_logger(ib_stream, "InnoDB: Error in pages %lu and %lu of ",
            buf_block_get_page_no(block1), buf_block_get_page_no(block2));
  dict_index_name_print(ib_stream, NULL, dict_index);
  if (level) {
    ib_logger(ib_stream, ", index tree level %lu", level);
  }
...
@return	true if ok */
static bool btr_validate_level(dict_index_t *dict_index, /*!< in: index tree */
...
loop:
  ...
  ut_a(page_get_n_recs(page) > 0 ||
       (level == 0 &&
        page_get_page_no(page) == dict_index_get_page(dict_index)));
  ...
  if (right_page_no != FIL_NULL) {
    ...
    if (unlikely(btr_page_get_prev(right_page, &mtr) !=
                 page_get_page_no(page))) {
      btr_validate_report2(dict_index, level, block, right_block);
      ib_logger(ib_stream, "InnoDB: broken FIL_PAGE_NEXT"
    ...
    if (unlikely(page_is_comp(right_page) != page_is_comp(page))) {
      btr_validate_report2(dict_index, level, block, right_block);
      ib_logger(ib_stream, "InnoDB: 'compact' flag mismatch\n");
      buf_page_print(page, 0);
      buf_page_print(right_page, 0);
    ...
    offsets = rec_get_offsets(rec, dict_index, offsets, ULINT_UNDEFINED, &heap);
    offsets2 = rec_get_offsets(right_rec, dict_index, offsets2, ULINT_UNDEFINED,
                               &heap);
    if (unlikely(cmp_rec_rec(rec, right_rec, offsets, offsets2, dict_index) >=
                 0)) {

      btr_validate_report2(dict_index, level, block, right_block);

      ib_logger(ib_stream, "InnoDB: records in wrong order"
                           " on adjacent pages\n");
  ...
  if (buf_block_get_page_no(block) != dict_index_get_page(dict_index)) {
    ...
    offsets = btr_page_get_father_block(offsets, heap, dict_index, block, &mtr,
                                        &node_cur);
    father_page = btr_cur_get_page(&node_cur);
    node_ptr = btr_cur_get_rec(&node_cur);

    btr_cur_position(dict_index, page_rec_get_prev(page_get_supremum_rec(page)),
                     block, &node_cur);
    offsets = btr_page_get_father_node_ptr(offsets, heap, &node_cur, &mtr);

    if (unlikely(node_ptr != btr_cur_get_rec(&node_cur)) ||
        unlikely(btr_node_ptr_get_child_page_no(node_ptr, offsets) !=
                 buf_block_get_page_no(block))) {
    ...
    if (!page_is_leaf(page)) {
      node_ptr_tuple = dict_index_build_node_ptr(
          dict_index, page_rec_get_next(page_get_infimum_rec(page)), 0, heap,
          btr_page_get_level(page, &mtr));

      if (cmp_dtuple_rec(dict_index->cmp_ctx, node_ptr_tuple, node_ptr,
                         offsets)) {
        ...
        ib_logger(ib_stream, "InnoDB: Error: node ptrs differ"
                             " on levels > 0\n"
                             "InnoDB: node ptr ");
    ...
    if (right_page_no == FIL_NULL) {
      ...
      offsets = btr_page_get_father_block(offsets, heap, dict_index,
                                          right_block, &mtr, &right_node_cur);
      if (right_node_ptr != page_get_supremum_rec(father_page)) {

        if (btr_cur_get_rec(&right_node_cur) != right_node_ptr) {
          ret = false;
          ib_logger(ib_stream, "InnoDB: node pointer to"
        ...
        if (btr_cur_get_rec(&right_node_cur) !=
          ...
          ret = false;
          ib_logger(ib_stream, "InnoDB: node pointer 2 to"
                               " the right page is wrong\n");
        ...
        if (page_get_page_no(right_father_page) !=
          ...
          ret = false;
          ib_logger(ib_stream, "InnoDB: node pointer 3 to"
                               " the right page is wrong\n");
...
bool btr_validate_index(dict_index_t *dict_index, /*!< in: index */
                        trx_t *trx) /*!< in: transaction or NULL */
{
  mtr_t mtr;
  page_t *root;
