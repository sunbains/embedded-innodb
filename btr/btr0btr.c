	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	return(btr_block_get(space, dict_table_zip_size(dict_index->table),
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	offsets = rec_get_offsets(node_ptr, dict_index, offsets,
	...
	if (UNIV_UNLIKELY(btr_node_ptr_get_child_page_no(node_ptr, offsets)
		...
		offsets = rec_get_offsets(print_rec, dict_index,
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	mlog_open_and_write_index(mtr, page, dict_index, page_is_comp(page)
	...
	if (UNIV_LIKELY_NULL(page_zip)
	    && UNIV_UNLIKELY
	    (!page_zip_compress(page_zip, page, dict_index, NULL))) {
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	if (0
		...
		page_zip_copy_recs(new_page_zip, new_page,
				   root_page_zip, root, dict_index, mtr);
	...
	node_ptr = dict_index_build_node_ptr(dict_index, rec, new_page_no, heap,
					     level);
	...
	page_cur_search(new_block, dict_index, tuple,
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	if (direction == FSP_DOWN) {
		...
		offsets = btr_page_get_father_block(NULL, heap, dict_index,
						    block, mtr, &cursor);
	...
	btr_insert_on_non_leaf_level(dict_index, level + 1, node_ptr_upper,
				     mtr);
	...
	mem_heap_t**	heap)	/*!< in/out: heap for offsets */
{
	...
	return(cmp_dtuple_rec(cursor->index->cmp_ctx, tuple, first_rec,
				offsets) < 0);
}
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	btr_page_empty(father_block, father_page_zip, dict_index,
		       page_level, mtr);
						...
						and page on error */
{
	...
	offsets = rec_get_offsets(rec, dict_index, offsets,
				  ULINT_UNDEFINED, &heap);

	for (i = 0; i < n; i++) {
		ulint	fixed_size = dict_col_get_fixed_size(
			dict_index_get_nth_col(dict_index, i),
			page_is_comp(page));
		...
		if ((dict_index_get_nth_field(dict_index, i)->prefix_len == 0
			...
			&& len != UNIV_SQL_NULL
			&& len
			> dict_index_get_nth_field(dict_index, i)->prefix_len)) {
	...
	ulint		level)	/*!< in: level number */
{
	...
	while (level != btr_page_get_level(page, &mtr)) {
		...
		offsets = rec_get_offsets(node_ptr, dict_index, offsets,
					  ULINT_UNDEFINED, &heap);
		block = btr_node_ptr_get_child(node_ptr, dict_index,
					       offsets, &mtr);
		page = buf_block_get_frame(block);
	}
...
loop:
	...
	ut_a(page_get_n_recs(page) > 0 || (level == 0
					   && page_get_page_no(page)
					   == dict_index_get_page(dict_index)));
	...
	if (right_page_no != FIL_NULL) {
		...
		if (UNIV_UNLIKELY(btr_page_get_prev(right_page, &mtr)
				  != page_get_page_no(page))) {
			btr_validate_report2(dict_index, level, block,
					     right_block);
		...
		right_rec = page_rec_get_next(page_get_infimum_rec(
						      right_page));
		offsets = rec_get_offsets(rec, dict_index,
					  offsets, ULINT_UNDEFINED, &heap);
		offsets2 = rec_get_offsets(right_rec, dict_index,
	...
	if (buf_block_get_page_no(block) != dict_index_get_page(dict_index)) {
		...
		if (!page_is_leaf(page)) {
			...
			if (cmp_dtuple_rec(dict_index->cmp_ctx,
		...
		if (right_page_no == FIL_NULL) {
			...
			offsets = btr_page_get_father_block(
				offsets, heap, dict_index, right_block,
				&mtr, &right_node_cur);
			if (right_node_ptr
			    != page_get_supremum_rec(father_page)) {
				...
				if (btr_cur_get_rec(&right_node_cur)
					...
					btr_validate_report1(dict_index, level,
							     block);
